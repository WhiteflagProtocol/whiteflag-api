...
const DSCONFFILE = './config/datastores.toml';
...
function initDatastores(callback) {
    ...
    if (parseConfig(datastoresConfig)) {
        ...
        log.info(MODULELOG, `Configuration read from ${DSCONFFILE}: ` + JSON.stringify(_datastores));
            ...
            if (dbConfig && dbConfig._enabled) {
                ...
                dbConfig._moduleImpl.init(dbConfig, function datastoreInitCb(err) {
                    ...
                    if (dbConfig.primary) {
                        ...
                        log.info(MODULELOG, `Connected to primary datastore: ${dbConfig.name}`);
 ...
 * @todo check the result before calling the callback
 */
function getMessages(wfQuery = {}, callback) {
    ...
    function datastoreGetMessagesCb(err, wfMessages, count) {
        ...
        // TODO: Check if result is an array
        // TODO: Add count of items in array
...
function enableDbConfig(dbConfig) {
    ...
    if (dbConfig.primary !== undefined) {
        ...
        try {
            ...
            log.warn(MODULELOG, `Module ${dbConfig.module} cannot be loaded: ${err.message}`);
            return false;
        }
        ...
        if (dbConfig.rxStoreEvent.length > -1 && dbConfig.txStoreEvent.length > -1) {
            if (dbConfig.primary) {
                const RXSTOREEVENT = 'messageProcessed';
                if (dbConfig.rxStoreEvent.indexOf(RXSTOREEVENT) < 0) {
                    dbConfig.rxStoreEvent.push(RXSTOREEVENT);
                    log.info(MODULELOG, `Auto configured primary datastore ${dbConfig.name} to store message upon '${RXSTOREEVENT}' rx events`);
                }
                const TXSTOREEVENT = 'messageProcessed';
                if (dbConfig.txStoreEvent.indexOf(TXSTOREEVENT) < 0) {
                    dbConfig.rxStoreEvent.push(TXSTOREEVENT);
                    log.info(MODULELOG, `Auto configured primary datastore ${dbConfig.name} to store message upon '${TXSTOREEVENT}' tx events`);
                }
            }
 ...
 * @private
 * @param {string} event the tx event to be checked with each enabled datastore
 * @param {object} wfMessage the sent whiteflag message
 */
function txStoreMessage(event, wfMessage) {
     ...
     * @param {object} err error object if any error
     * @param {object} storedMessage the stored Whiteflag message
     */
 ...
 * @param {object} wfMessage the received whiteflag message
 */
