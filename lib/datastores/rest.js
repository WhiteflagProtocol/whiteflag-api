...
function storeMessage(wfMessage, callback) {
    log.trace(_datastore, `Storing message: ${wfMessage.MetaHeader.transactionHash}`);
...
function getMessages(wfQuery, callback) {
    log.trace(_datastore, 'Performing message query: ' + JSON.stringify(wfQuery));
...
function storeState(stateObject, callback) {
    log.trace(_datastore, 'Storing state');
...
function getState(callback) {
    log.trace(_datastore, 'Retrieving state');
