...
function storeMessage(wfMessage, callback) {
    log.trace(_dbName, `Upserting message in collection ${MESSAGECOLLECTION}: ${wfMessage.MetaHeader.transactionHash}`);
...
function getMessages(wfQuery, callback) {
    ...
    cursor.each(function mongodbGetMessagesCb(err, wfMessage) {
        if (!wfMessage) {
            // No more messages
            log.trace(_datastore, `Found ${count} messages for query: ${JSON.stringify(wfQuery)}`);
