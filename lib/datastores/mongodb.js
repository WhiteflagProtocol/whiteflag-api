...
function storeMessage(wfMessage, callback) {
    ...
    const query = { 'MetaHeader.transactionHash': wfMessage.MetaHeader.transactionHash };
    _dbMessagesCollection.findOne(query, function mongodbPreserveTransceiveCb(err, message) {
        // Preserve transceive direction
        if (!err && message?.MetaHeader?.transceiveDirection) {
            const transceiveDirection = message.MetaHeader.transceiveDirection
                                        || wfMessage.MetaHeader.transceiveDirection;
            log.trace(_dbName, `Preserving transceive direction of message ${message.MetaHeader.transactionHash}: ${transceiveDirection}`);
            wfMessage.MetaHeader.transceiveDirection = transceiveDirection;
        }
        // Upsert message
