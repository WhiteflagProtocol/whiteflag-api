...
const { determineStartingBlock } = require('../common');
...
async function initListener(ethConfig, ethState) {
    ...
    try {
        _ethState.status.highestBloc = await ethRpc.getHighestBlock();
        _blockCursor = await determineStartingBlock(
            _ethState.status.highestBlock,
            _ethState.status.currentBlock,
            _blockRetrievalStart,
            _blockRetrievalRestart
        );
        logStartingBlock(_blockCursor, _ethState.status.highestBlock);
    } catch(err) {
        if (err) return Promise.reject(err);
        return Promise.reject(new Error('Could not connect to Ethereum node and start blockchain listener'));
    }
...
function executeBlockIteration() {
    ...
    .then(async () => {
        ...
        _blockCursor = await determineStartingBlock(_ethState.status.highestBlock, _blockCursor, _blockRetrievalEnd, _blockRetrievalRestart);
        logStartingBlock(_blockCursor, _ethState.status.highestBlock);
 ...
 * Processes a blocks
 * @private
 * @param {number} thisBlock the block to be processed
 * @returns {Promise} the result, which may be an Error or null (success)
 */
async function processBlock(thisBlock) {
    // Get block from node
    let block;
    try {
        block = await ethRpc.getBlockByNumber(thisBlock);
    } catch(err) {
        return Promise.reject(new Error(`Could not retrieve block ${thisBlock}: ${err.message}`));
    }
    ...
    log.trace(_blockchainName, `Transactions discovered in block ${thisBlock}: ${transactionCount}`);
    return processTransactions(0, block.transactions, block.timestamp)
    .then(() => {
        log.info(_blockchainName, `Transactions processed from block ${thisBlock}: ${transactionCount} `);
        return Promise.resolve(); // Completed this block
    })
    .catch(err => {
        if (!err) return Promise.reject(new Error(`Could not process block ${thisBlock}`));
        return Promise.reject(new Error(`Could not process block ${thisBlock}: ${err.message}`));
    });
}
 ...
 * @returns {Error} the result, which may be an Error or null (success)
 */
 function processTransactions(index, transactions, timestamp) {
    // Get transaction batch of Promises in an array
    let transactionBatch = getTransactionBatch(index, transactions, timestamp);
    ...
    .then(data => {
        ignore(data); // Any Whiteflag message is already succesfully extracted by the promise
 ...
 */
function getTransactionBatch(index, transactions, timestamp) {
    ...
    for (
        ...
        transactionBatch.push(
            ...
            .then(transaction => {
                ...
                if (_traceRawTransaction) log.trace(_blockchainName, `Processing transaction: ${transactionHash}`);
 ...
 */
function logStartingBlock(blockCursor, highestBlock) {
    let nextBlock = blockCursor + 1;
    let arrearBlocks = highestBlock - nextBlock;
    if (arrearBlocks < 0) {
        log.info(_blockchainName, `Resuming with next block: ${nextBlock} (when node catches up from its current highest block ${highestBlock})`);
    } else {
        log.info(_blockchainName, `Resuming with next block: ${nextBlock} (${arrearBlocks} blocks behind the highest known block ${highestBlock})`);
    }
}
