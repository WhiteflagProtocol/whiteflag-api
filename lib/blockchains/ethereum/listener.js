...
async function initListener(ethConfig, ethState) {
    log.trace(_ethName, 'Initialising listener for Ethereum blockchain transactions...');
    _ethName = ethConfig.name;
    _ethState = ethState;

    // Block processing parameters
    if (ethConfig.blockMaxRetries) _blockMaxRetries = ethConfig.blockMaxRetries;
    if (_blockMaxRetries > 0) log.debug(_ethName, `Maximum retries for processing a block is set to ${_blockMaxRetries} for each block`);
    if (ethConfig.transactionBatchSize) _transactionBatchSize = ethConfig.transactionBatchSize;
    log.debug(_ethName, `Maximum number of transactions in a block that are processed in parallel is set to ${_transactionBatchSize}`);

    // Block interval time
    if (ethConfig.blockRetrievalInterval && ethConfig.blockRetrievalInterval > 500) {
        _blockInterval = ethConfig.blockRetrievalInterval;
    }
    log.info(_ethName, `Block retrieval interval: ${_blockInterval} ms`);
    ...
    if (ethConfig.blockRetrievalStart && ethConfig.blockRetrievalStart > 0) {
        log.info(_ethName, `Starting block specified in configuration: ${ethConfig.blockRetrievalStart}`);
        _blockRetrievalStart = ethConfig.blockRetrievalStart;
    }
    if (ethConfig.blockRetrievalEnd && ethConfig.blockRetrievalEnd > 0) {
        log.info(_ethName, `Ending block specified in configuration: ${ethConfig.blockRetrievalEnd}`);
    ...
    try {
        ...
        logStartingBlock(_ethName, _blockCursor, _ethState.status.highestBlock);
    } catch(err) {
        if (err) return Promise.reject(err);
        return Promise.reject(new Error('Could not connect to Ethereum node and determine starting block'));
    }
...
function scheduleBlockIteration() {
    log.trace(_ethName, `Scheduling block retrieval iteration ${(_iterationCount + 1)} in ${_blockInterval} ms`);
...
function executeBlockIteration() {
    _iterationCount += 1;
    log.trace(_ethName, `Starting block retrieval iteration ${_iterationCount}`);
    ...
    .then(highestBlock => {
        ...
        if (highestBlock >  _discoveredBlock) {
            ...
            log.trace(_ethName, `New highest block discovered on node is ${_discoveredBlock}`);
        }
        // Check if highest block is already processed, and schedule next iteration
        if (highestBlock === _blockCursor) {
            log.trace(_ethName, `Highest block ${highestBlock} has already been processed`);
        ...
        if (_blockCursor > highestBlock) {
            log.debug(_ethName, `Current block ${_blockCursor} is higher than highest block ${highestBlock} on node`);
    ...
    .then(async () => {
        ...
        log.info(_ethName, `Reached configured block retrieval end: ${_blockCursor}`);
    ...
    .catch(err => {
        if (err) {
            ...
            log.warn(_ethName, `Could not complete retrieval of block ${_blockCursor} in iteration ${_iterationCount}: ${err.message}`);
...
async function processBlocks(startBlock, endBlock) {
    ...
    if (_blockStackSize > BLOCKSTACKSIZE) {
        log.trace(_ethName, `Reached maximum block processing stack size: ${_blockStackSize}`);
        return Promise.resolve(); // Completed for now
    }
    // If current block is last block, then there is no new block to retrieve
    if (startBlock === endBlock) {
        log.trace(_ethName, 'No new block to retrieve');
        return Promise.resolve(); // Completed for now
    }
    // Block to be processed is the next block after starting block and skipped blocks
    if (_skippedBlocks > 0) log.trace(_ethName, `Skipped ${_skippedBlocks} blocks since block ${startBlock}`);
    ...
    if (_blockMaxRetries > 0 && _blockRetryCount > _blockMaxRetries) {
        log.warn(_ethName, `Skipping block ${thisBlock} after ${_blockMaxRetries} retries`);
        thisBlock += 1;
        _blockRetryCount = 0;
        _skippedBlocks += 1;
    }
    // Log where we are with blocks
    if (_blockRetryCount !== 0) log.info(_ethName, `Retry ${_blockRetryCount} to process block: ${thisBlock}`);
    log.trace(_ethName, `Retrieving block ${_blockStackSize} of iteration ${_iterationCount}: ${thisBlock}`);
...
async function processBlock(blockNumber) {
    ...
    if (!block || transactionCount < 1) {
        ...
        log.info(_ethName, `No transactions in block: ${blockNumber}`);
        return Promise.resolve(); // Completed this block
    }
    // Retrieve transactions from block
    log.trace(_ethName, `Transactions discovered in block ${blockNumber}: ${transactionCount}`);
    return processTransactions(0, block.transactions, block.timestamp)
    .then(() => {
        log.info(_ethName, `Transactions processed from block ${blockNumber}: ${transactionCount} `);
        return Promise.resolve(); // Completed this block
    })
    .catch(err => {
...
function createTransactionBatch(index, transactions, timestamp) {
    ...
    for (
        ...
        transactionBatch.push(
            ...
            .then(transaction => {
                if (!transaction) return log.warn(_ethName, `No data received for transaction: ${transactionHash}`);
                return ethTransactions.extractMessage(transaction, timestamp);
            })
            .then(wfMessage => {
                log.trace(_ethName, `Received Whiteflag message: ${JSON.stringify(wfMessage.MetaHeader)}`);
