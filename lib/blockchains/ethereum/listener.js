...
async function initListener(ethConfig, ethState) {
    ...
    try {
        ...
        blockchainUtil.logStartingBlock(_blockchainName, _blockCursor, _ethState.status.highestBlock);
    } catch(err) {
...
function executeBlockIteration() {
    ...
    .then(async () => {
        ...
        _blockCursor = blockchainUtil.determineStartingBlock(_ethState.status.highestBlock, _blockCursor, _blockRetrievalEnd, _blockRetrievalRestart);
        blockchainUtil.logStartingBlock(_blockchainName, _blockCursor, _ethState.status.highestBlock);
