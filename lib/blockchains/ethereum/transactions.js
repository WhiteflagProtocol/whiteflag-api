...
function initTransactions(ethConfig, ethState, web3) {
    log.trace(_blockchainName, 'Initialising Ethereum transactions processing...');
 ...
 */
 function extractMessage(transaction, timestamp) {
    if (_traceRawTransaction) log.trace(_blockchainName, `Processing transaction: ${transaction.hash}`);
    return new Promise((resolve, reject) => {
        if (
            !transaction ||
            !transaction.input ||
