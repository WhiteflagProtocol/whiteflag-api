    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            if (privateKey) {
                ...
                account = createAccountEntry(ethAccounts.privateKeyToAccount(withHexPrefix(privateKey)));
                updateAccountBalance(account);
                updateAccountTransactions(account);
            } else {
                log.trace(MODULELOG, `Creating new ${_ethChain} account with generated keys`);
                account = createAccountEntry(ethAccounts.create(crypto.randomBytes(64)));
                account.balance = 0;
...
function updateAccountBalance(account) {
    ...
    .then(balance => {
        account.balance = ethUtils.fromWei(balance, 'ether');
...
function upsertAccount(account) {
    ...
    if (Object.hasOwn(account, 'privateKey')) {
        const privateKeyId = getPrivateKeyId(_ethName, noAddressHexPrefix(account.address));
 ...
 */
 function createAccountEntry(ethAccount) {
    const publicKey = ethAccounts.privateKeyToPublicKey(hexToBuffer(noKeyHexPrefix(ethAccount.privateKey)));
    return {
