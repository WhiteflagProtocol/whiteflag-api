...
async function initAccounts(ethConfig, ethState, web3) {
    ...
    _ethState = ethState;
    _ethApi = web3;
    log.trace(_ethName, 'Initialising Ethereum accounts management...');

    // If configured, create new account if none exists
    if (ethConfig.createAccount && _ethState.accounts.length === 0) {
        createAccount(null)
        .then(account => {
            log.info(_ethName, `Ethereum account created automatically: ${account.address}`);
        })
        .catch(err => log.warn(_ethName, err.message));
    }
...
function createAccount(privateKey) {
    return new Promise((resolve, reject) => {
        ...
        try {
            if (privateKey) {
                log.trace(_ethName, 'Creating Ethereum account from existing private key');
                account = generateAccount(_ethApi.eth.accounts.privateKeyToAccount(withHexPrefix(privateKey)));
                updateAccountBalance(account);
                updateAccountTransactions(account);
            } else {
                log.trace(_ethName, 'Creating new Ethereum account with generated keys');
                account = generateAccount(_ethApi.eth.accounts.create(crypto.randomBytes(64)));
                account.balance = 0;
            ...
            privateKey = undefined;
            log.error(_ethName, `Error while creating new Ethereum account: ${err.message}`);
            return reject(err);
        }
...
function updateAccount(account) {
    log.trace(_ethName, `Updating Ethereum account: ${account.address}`);
    return new Promise((resolve, reject) => {
...
function deleteAccount(address) {
    log.trace(_ethName, `Deleting Ethereum account: ${address}`);
    return new Promise((resolve, reject) => {
...
function updateAccountBalance(account) {
    ...
    .catch(err => log.warn(_ethName, `Could not update balance for account ${account.address}: ${err.message}`));
}
...
function updateAccountTransactions(account) {
    ...
    .catch(err => log.warn(_ethName, `Could not update transaction count for account ${account.address}: ${err.message}`));
}
...
function upsertAccount(account) {
    ...
    if (!existingAccount) {
        ...
        log.trace(_ethName, `New Ethereum account added to state: ${account.address}`);
    } else {
        // Update account
        object.update(account, existingAccount);
        log.trace(_ethName, `Ethereum account updated in state: ${account.address}`);
    }
    wfState.updateBlockchainData(_ethName, _ethState);
}
 ...
 */
 function generateAccount(newEthAccount) {
    const publicKey = ethereumUtil.privateToPublic(Buffer.from(noKeyHexPrefix(newEthAccount.privateKey), BINENCODING));
    return {
        address: noAddressHexPrefix(newEthAccount.address),
        publicKey: noPubkeyHexPrefix(publicKey.toString(BINENCODING)),
        privateKey: noKeyHexPrefix(newEthAccount.privateKey),
        balance: null,
