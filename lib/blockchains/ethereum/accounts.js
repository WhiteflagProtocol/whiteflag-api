...
function updateAccountBalance(account) {
    ...
    .catch(err => log.warn(_blockchainName, `Could not update balance for Ethereum account ${account.address}: ${err.message}`));
}
...
function updateAccountTransactions(account) {
    ...
    .catch(err => log.warn(_blockchainName, `Could not update transaction count for Ethereum account ${account.address}: ${err.message}`));
}
