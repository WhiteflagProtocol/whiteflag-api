...
const { withHexPrefix } = require('../common/format');
...
function initRpc(ethConfig, ethState) {
    ...
    log.info(_ethName, `Timeout for remote calls to the Ethereum node: ${_rpcTimeout} ms`);

    // Get Node URL and credentials
    const rpcAuthURL = getFullURL(ethConfig, false);    // include credentials
    const rpcCleanURL = getFullURL(ethConfig, true);    // no credentials
    _ethState.parameters.rpcURL = rpcCleanURL;
    _ethApi = new Web3(rpcAuthURL);
    log.info(_ethName, `Using web3 Ethereum API version: ${_ethApi.version}`);

    // Connect to the Ethereum node
    log.trace(_ethName, `Setting up connection with Ethereum node: ${rpcCleanURL}`);
    return new Promise((resolve, reject) => {
        ...
        .then(chainID => {
            ...
            log.info(_ethName, `Connected to Ethereum chain ${chainID} through node: ${rpcCleanURL}`);
        ...
        .catch(err => reject(new Error(`Could not connect to Ethereum node: ${err.message}`), _ethName));
    });
}
 ...
 */
 function getFullURL(ethConfig, hideCredentials = false) {
    ...
    let rpcAuth = '';
    if (ethConfig.rpcUsername && ethConfig.rpcPassword && !hideCredentials) {
        rpcAuth = ethConfig.rpcUsername + ':' + ethConfig.rpcPassword + '@';
    }
...
async function updateNodeInfo() {
    ...
    .catch(err => log.warn(_ethName, `Could not get node software and version information: ${err.message}`));

    // Get protocol version
    await getProtocolVersion()
    .then(protocolVersion => (_ethState.parameters.protocolVersion = protocolVersion))
    .catch(err => log.warn(_ethName, `Could not get protocol version from node: ${err.message}`));

    // Get network ID
    await getNetworkId()
    .then(networkID => (_ethState.parameters.networkID = networkID))
    .catch(err => log.warn(_ethName, `Could not get Network ID from node: ${err.message}`));

    // Check chain ID
    await getChainId()
    .then(chainID => {
        if (chainID !== _chainID) {
            log.warn(_ethName, `The node's Chain ID has changed to ${chainID} and does not correspond with the configured Chain ID ${_chainID}`);
        }
        _ethState.parameters.chainID = chainID;
    })
    .catch(err => log.warn(_ethName, `Could not get Chain ID from node: ${err.message}`));
}
...
async function updateNodeStatus() {
    ...
    .catch(err => log.warn(_ethName, `Could not get peer status: ${err.message}`));

    // Get synchronisation status
    await isSyncing()
    .then(syncing => (_ethState.status.syncing = syncing))
    .catch(err => log.warn(_ethName, `Could not get synchronisation status: ${err.message}`));

    // Get gas price
    await getGasPrice()
    .then(gasPrice => (_ethState.status.gasPrice = gasPrice))
    .catch(err => log.warn(_ethName, `Could not get gas price: ${err.message}`));
...
function saveNodeStatus() {
    ...
    log.info(_ethName, `Status: {${JSON.stringify(_ethState.parameters)}, ${JSON.stringify(_ethState.status)}}`);
}
