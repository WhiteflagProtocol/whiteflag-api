...
const LOGSTATUSDELAY = 1000; // One second
...
function initRpc(ethConfig, ethState) {
    ...
    return new Promise((resolve, reject) => {
        ...
        .then(chainID => {
            ...
            log.info(_blockchainName, `Connected to Ethereum chain ${_chainID} through node: ${rpcCleanURL}`);
...
function logNodeStatus() {
    ...
    log.info(_blockchainName, `Node status: {${JSON.stringify(_ethState.parameters)}, ${JSON.stringify(_ethState.status)}}`);
}
