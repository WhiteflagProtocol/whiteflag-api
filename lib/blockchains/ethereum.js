...
function sendTransaction(account, toAddress, value, data, callback) {
    ...
    createTransaction(account, toAddress, value, data, function ethCreateTransactionCb(err, rawTransactionObject) {
        ...
        wfState.getKey('blockchainKeys', privateKeyId, function ethGetKeyCb(err, privateKey) {
            ...
            // NOTE: the web3 API is not stable and might not fire a receipt event; therefore extra checks
            ...
            function sendTransactionConfirmation(confNumber, receipt) {
                if (!transactionConfirmation) {
                    ...
                    if (confNumber > 2 && transactionHash) {
                        log.debug(_blockchainName, `Sending raw transaction ${transactionHash} considered successful after ${confNumber} confirmations without receipt`);
                        transactionConfirmation = true;
                        return sendTransactionSuccess(receipt);
                    }
                    return log.debug(_blockchainName, `Received ${confNumber} confirmations without receipt for transaction ${transactionHash}`);
                }
                return log.trace(_blockchainName, `Received ${confNumber} confirmations for transaction ${transactionHash}`);
            }
            /**
            ...
            function sendTransactionSuccess(receipt) {
                if (!transactionCompleted) {
                    ...
                    if (transactionHash) return callback(null, transactionHash);
            ...
            function sendTransactionCheckResult(result) {
                if (!transactionCompleted) {
                    ...
                    if (result.receipt) {
                        log.warn(_blockchainName, `Unspecified condition occured, but received receipt for transaction ${apiFormatHex(result.receipt.transactionHash)}: ${JSON.stringify(result)}`);
                        return sendTransactionSuccess(result.receipt);
                    }
                    if (result.confirmations && transactionHash) {
                        log.warn(_blockchainName, `Unspecified condition occured, but received confirmation for transaction ${transactionHash}: ${JSON.stringify(result)}`);
                        return sendTransactionConfirmation(result.confirmations, result.receipt);
                    }
                    // Assume success if transcation hash is known
                    if (transactionHash) {
                        log.warn(_blockchainName, `Unspecified condition occured, but assuming success for transaction ${transactionHash}: ${JSON.stringify(result)}`);
                        return sendTransactionSuccess(null);
                    }
                    // Assume error if nothing is known
                    return sendTransactionError(new Error(`Unspecified condition while sending raw transaction: ${JSON.stringify(result)}`));
                }
                // Otherwise it is unclear
                return log.trace(_blockchainName, `Unspecified condition after sending raw transaction: ${JSON.stringify(result)}`);
            }
        });
    });
}
