 ...
 */
function verifySignature(wfSignature, ethPublicKey, callback) {
    log.trace(MODULELOG, `Validating signature: ${JSON.stringify(wfSignature)}`);
    let wfSignDecoded;
    try {
        wfSignDecoded = verifySignature(wfSignature, ethPublicKey, SIGNKEYTYPE);
    } catch(err) {
        log.error(MODULELOG, `Could not not verify signature: ${err.message}`);
        return callback(err);
    }
    return callback(null, wfSignDecoded, ethPublicKey);
}
