...
function requestKeys(publicKey, callback) {
    ...
    try {
        ...
        if (originatorKeys.publicKey.hex.length === 130) keccakPubkeyBuffer = Buffer.from(originatorKeys.publicKey.hex.substring(2), BINENCODING);
        ...
        // Ethereum Address
        originatorKeys.address = noAddressHexPrefix(ethereumUtil.pubToAddress(publicKeyBuffer, true).toString(BINENCODING));
    } catch(err) {
...
function getBinaryAddress(address, callback) {
    if (_web3.utils.isAddress(withHexPrefix(address))) {
