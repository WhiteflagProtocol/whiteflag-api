...
function initEthereum(bcConfig, callback) {
    ...
    wfState.getBlockchainData(_blockchainName, function blockchainsGetStateDb(err, blockchainState) {
        ...
        .then(function web3GetNetIdResolve(networkID) {
            ...
            log.info(_blockchainName, `Configured Ethereum Chain ID: ${_chainID}`);
        ...
        .then(function web3GetBlockNumberResolve(highestBlock) {
            ...
            if (bcConfig.blockRetrievalStart && bcConfig.blockRetrievalStart > 0) {
                _currentBlock = bcConfig.blockRetrievalStart - 1;
                log.info(_blockchainName, `Current block is set to starting block in configuration: ${bcConfig.blockRetrievalStart}`);
            ...
            if (_currentBlock < 1) {
                if (_blockchainState.status.currentBlock > 0) {
                    if ((highestBlock - _blockchainState.status.currentBlock) > _blockRetrievalRestart) {
                        _currentBlock = highestBlock - _blockRetrievalRestart;
                        log.info(_blockchainName, `Resuming from block: ${_currentBlock} (${(highestBlock - _currentBlock)} blocks before highest block ${highestBlock})`);
                    }
                    if ((highestBlock - _blockchainState.status.currentBlock) < _blockRetrievalRestart) {
                        _currentBlock = _blockchainState.status.currentBlock;
                        log.info(_blockchainName, `Resuming from current block: ${_currentBlock} (${(highestBlock - _currentBlock)} blocks before highest block ${highestBlock})`);
                    }
                } else {
                    _currentBlock = highestBlock - 1;
                    log.info(_blockchainName, `Starting from block: ${_currentBlock}`);
                }
            }
            ...
            return callback(new Error(`Cannot initialise transaction listener because could not get current highest block: ${err.message}`), _blockchainName);
        });
    });
}
 ...
 * @param {object} wfMessage the Whiteflag message to be sent on Ethereum
...
function sendMessage(wfMessage, callback) {
    ...
    if (!account) return callback(new ProcessingError(`No ${_blockchainName} account with address: ${wfMessage.MetaHeader.originatorAddress}`));
...
function lookupMessage(wfQuery, callback) {
    ...
    .then(function ethGetTransactionResolve(transaction) {
        ...
        return callback(new Error(`Cannot retrieve Whiteflag message: ${err.message}`));
...
function getTransaction(transactionHash, callback) {
    ...
    .then(function ethGetRawTransactionResolve(transaction) {
        ...
        return callback(new Error(`Cannot retrieve raw transaction: ${err.message}`));
...
function requestSignature(signPayload, callback) {
    ...
    if (!account) return callback(new ProcessingError(`No ${_blockchainName} account with address: ${signPayload.addr}`));
    ...
    wfState.getKey('blockchainKeys', privateKeyId, function ethGetKeyCb(err, privateKey) {
        ...
        try {
            ...
            log.error(_blockchainName, `Cannot not sign payload: ${err.message}`);
            return callback(err);
        }
...
function requestKeys(originatorPubKey, callback) {
    ...
    try {
        ...
        log.error(_blockchainName, `Cannot get key and address: ${err.message}`);
        return callback(err);
    }
...
function getBinaryAddress(address, callback) {
    // TODO: Check if valid Ethereum address
...
function transfer(transfer, callback) {
    ...
    if (!account) return callback(new ProcessingError(`No ${_blockchainName} account with address: ${transfer.fromAddress}`));
...
function createAccount(privateKey, callback) {
    ...
    const privateKeyId = hash(_blockchainName + account.address, KEYIDLENGTH);
    wfState.storeKey('blockchainKeys', privateKeyId, apiFormatHex(newEthAccount.privateKey));
...
function updateAccount(account, callback) {
    if (!getAccount(account.address)) {
        return callback(new ProcessingError(`No ${_blockchainName} account with address: ${account.address}`), null, 'WF_API_NO_RESOURCE');
    }
    // Securely store the private key in state
    if (account.privateKey) {
        const privateKeyId = hash(_blockchainName + account.address, KEYIDLENGTH);
        wfState.storeKey('blockchainKeys', privateKeyId, apiFormatHex(account.privateKey));
    ...
    try {
        ...
        return callback(new Error(`Cannot update ${_blockchainName} account: ${err.message}`));
    }
...
function deleteAccount(address, callback) {
    ...
    if (index < 0) {
        return callback(new ProcessingError(`No ${_blockchainName} account with address: ${address}`, null, 'WF_API_NO_RESOURCE'));
    }
    ...
    if (account.address === address) {
        ...
        return callback(new Error(`Cannot not delete account with address: ${address}`));
    }
    // Log and return result
 ...
 * @returns {URL}
 */
function getNodeURL(bcConfig, hideCredentials) {
    if (!hideCredentials) hideCredentials = false;
...
function updateAccounts() {
    _blockchainState.accounts.forEach(account => {
        getBalance(account.address, function ethUpdateBalanceCb(err, balance) {
            if (err) return log.warn(_blockchainName, `Cannot update balance for account ${account.address}: ${err.message}`);
        ...
        getTransactionCount(account.address, function ethUpdateTxCountCb(err, transactionCount) {
            if (err) return log.warn(_blockchainName, `Cannot update transaction count for account ${account.address}: ${err.message}`);
...
function sendTransaction(account, toAddress, value, data, callback) {
    ...
    createTransaction(account, toAddress, value, data, function ethCreateTransactionCb(err, rawTransactionObject) {
        ...
        wfState.getKey('blockchainKeys', privateKeyId, function ethGetKeyCb(err, privateKey) {
            ...
            // NOTE: the web3 API is not stable and might not return a receipt even when the transaction is succesful
            let transactionCompleted = false;
            let transactionConfirmation = false;
            let transactionHash = '';
            _web3.eth.sendSignedTransaction(rawTransaction)
            .once('transactionHash', function web3SendSignedTransactionHash(hash) {
                transactionHash = hash;
            })
            .once('receipt', sendTransactionSuccess)
            .on('confirmation', sendTransactionConfirmation)
            .on('error', sendTransactionError)
            .then(sendTransactionSuccess, sendTransactionError)
            .catch(function sendTransactionError(err) {
                return callback(new Error(`Unhandled rejection while sending raw transaction: ${err.message}`));
            });
            /**
             * Invokes callback when transaction is confirmed
             * @private
             */
            function sendTransactionConfirmation(confNumber, receipt) {
                if (!transactionConfirmation) {
                    ...
                    if (!receipt) {
                        log.debug(_blockchainName, `Received confirmation ${confNumber} but no receipt for transaction ${transactionHash}`);
                        return sendTransactionSuccess(null);
                    }
                    log.debug(_blockchainName, `Received receipt for transaction ${apiFormatHex(receipt.transactionHash)} upon confirmation ${confNumber}`);
                    return sendTransactionSuccess(receipt);
                }
                if (receipt) return log.trace(_blockchainName, `Received receipt for transaction ${apiFormatHex(receipt.transactionHash)} upon confirmation ${confNumber}`);
                if (!receipt) return log.trace(_blockchainName, `Received confirmation ${confNumber} but no receipt for transaction ${transactionHash}`);
            }
            /**
             * Invokes callback when transaction is completed /resolved
             * @private
             */
            function sendTransactionSuccess(receipt) {
                if (!transactionCompleted) {
                    transactionCompleted = true;
                    if (receipt) return callback(null, apiFormatHex(receipt.transactionHash), receipt.blockNumber);
                    if (transactionHash) return callback(null, apiFormatHex(transactionHash));
                    return callback(null);
                }
            }
            /**
             * Invokes callback upon error / rejection
             * @private
             */
            function sendTransactionError(err) {
                if (!transactionCompleted) {
                    // Check for real error object
                    if (err.message) {
                        transactionCompleted = true;
                        return callback(new Error(`Cannot send raw transaction: ${err.message}`));
                    }
                    // For some reason the error might be an object with receipt and confirmations instead
                    if (err.receipt) {
                        log.warn(_blockchainName, `Received receipt for transaction ${apiFormatHex(err.receipt.transactionHash)} upon an unspecified condition: ${JSON.stringify(err)}`);
                        return sendTransactionSuccess(err.receipt);
                    }
                    if (err.confirmations > 0) {
                        log.warn(_blockchainName, `Received confirmation without receipt for transaction ${transactionHash} upon an unspecified condition: ${JSON.stringify(err)}`);
                        return sendTransactionSuccess(null);
                    }
                    if (transactionHash) {
                        log.warn(_blockchainName, `Unspecified condition while sending transaction ${transactionHash}: ${JSON.stringify(err)}`);
                        return sendTransactionSuccess(null);
                    }
                    // Otherwise it is unclear
                    transactionCompleted = true;
                    return callback(new Error(`Unspecified condition while sending transaction from account ${account.address}: ${JSON.stringify(err)}`));
                }
            }
        });
    });
...
function getBlockchainData() {
    ...
    .then(function web3GetBlockNumberResolve(highestBlock) {
        ...
        if (highestBlock === _currentBlock) {
            log.trace(_blockchainName, `Highest block ${highestBlock} has already been processed`);
            return scheduleBlockchainData();
        }
        // Current block cannot be higher than highest block
        if (_currentBlock > highestBlock) {
            log.warn(_blockchainName, `Current block ${_currentBlock} is higher than highest block ${highestBlock}`);
            _currentBlock = highestBlock - _blockRetrievalRestart;
            log.info(_blockchainName, `Resuming from block: ${_currentBlock} (${(highestBlock - _currentBlock)} blocks before highest block ${highestBlock})`);
            return scheduleBlockchainData();
        }
        ...
        if (_blockRetrievalEnd === 0 && highestBlock !== _discoveredBlock) {
            ...
            log.trace(_blockchainName, `New highest block discovered: ${_discoveredBlock}`);
        }
        // Last block to retrieve is highest block or, if provided, end block
        let lastBlock = highestBlock;
        if (_blockRetrievalEnd > 0 && _blockRetrievalEnd < highestBlock) {
            lastBlock = _blockRetrievalEnd;
            log.info(_blockchainName, `Last block is set to ending block in configuration: ${lastBlock}`);
        }
        // Process blocks sequentially while counting iterations
        _blockStackSize = 0;
        getBlocks(_currentBlock, lastBlock, function ethGetBlocksCb(err) {
            if (err) {
            ...
            if (_currentBlock === _blockRetrievalEnd && _blockRetrievalEnd < highestBlock) {
                log.info(_blockchainName, `Reached configured block retrieval end: ${_blockRetrievalEnd}`);
                _blockRetrievalEnd = 0;

                // Determine from where to pick up the blockchain after end
                if ((highestBlock - _currentBlock) > _blockRetrievalRestart) {
                    _currentBlock = highestBlock - _blockRetrievalRestart;
                    log.info(_blockchainName, `Resuming from block: ${_currentBlock} (${(highestBlock - _currentBlock)} blocks before highest block ${highestBlock})`);
                }
                if ((highestBlock - _currentBlock) < _blockRetrievalRestart) {
                    log.info(_blockchainName, `Resuming from current block: ${_currentBlock}`);
                }
                // Start normal operation by scheduling next iteration
                return scheduleBlockchainData();
            }
            // Continue processing and get next set of blocks
            return getBlockchainData();
        });
    }, function web3GetBlockNumberReject(err) {
        log.warn(_blockchainName, `Cannot determine current highest block number: ${err.message}`);
        return scheduleBlockchainData();
    }).catch(function web3GetBlockError(err) {
        log.warn(_blockchainName, `Could not retrieve blockchain data: ${err.message}`);
        return scheduleBlockchainData();
    });
}
 ...
 * @private
 * @param {number} currentBlock the block up to which transactions are fully processed
 * @param {number} lastBlock the block up to which blocks should be processed
 * @param {function} callback function to call upon completion
 */
function getBlocks(currentBlock, lastBlock, callback) {
    ...
    if (currentBlock === lastBlock) {
        log.trace(_blockchainName, `Block ${lastBlock} already processed`);
        return callback(null);
    }
    // New block is the next block from the current (latest) block
    ...
    if (_blockMaxRetries > 0 && _blockRetryCount > _blockMaxRetries) {
        // Skip blocks
        _skippedBlocks += 1;
        newBlock += _skippedBlocks;
        log.warn(_blockchainName, `Skipping block ${(newBlock - 1)} after ${(_blockRetryCount - 1)} retries`);
        log.warn(_blockchainName, `Skipped ${_skippedBlocks} blocks since block ${currentBlock}`);
    ...
    if (_blockRetryCount === 0) {
        log.debug(_blockchainName, `Processing block ${newBlock} in iteration ${_iterationCount}/${_blockStackSize}`);
    } else {
        log.debug(_blockchainName, `Retry ${_blockRetryCount} to process block ${newBlock} in iteration ${_iterationCount}/${_blockStackSize}`);
    }
    // Retrieve new block under a timeout
    timeoutPromise(_web3.eth.getBlock(newBlock), _rpcTimeout)
    .then(function ethGetBlockResolve(block) {
        if (block && block.transactions.length > 0) {
            log.trace(_blockchainName, `Transactions discovered in block ${newBlock}: ${block.transactions.length}`);
            ...
            getTransactions(0, block.transactions, block.timestamp, function ethGetTransactionsCb(err) {
                ...
                if (err) {
                    _blockRetryCount += 1;
                    return callback(new Error(`Could not retrieve transactions in block ${newBlock}: ${err.message}`));
                }
                // Successfully processed transactions in block
                log.debug(_blockchainName, `Transactions processed from block ${newBlock}: ${block.transactions.length} `);
                return nextBlock();
            });
        } else {
            // Rare case, but in certain instances there could be no transactions in the block
            log.debug(_blockchainName, `No transactions in block ${newBlock}`);
            return nextBlock();
        }
    }).catch(function ethGetBlockError(err) {
        _blockRetryCount += 1;
        if (!err) err = new Error('Unknown error');
        return callback(new Error(`Could not retrieve data from block ${newBlock}: ${err.message}`));
    });
    /**
    ...
    function nextBlock() {
        ...
        // Update state
        _blockchainState.status.currentBlock = _currentBlock;
        wfState.updateBlockchainData(_blockchainName, _blockchainState);

        // Get next block if not yet at last block
        if (newBlock < lastBlock) {
            return getBlocks(newBlock, lastBlock, function getBlocksCb(err) {
                if (err) return callback(err);
                return callback(null);
            });
        } else {
