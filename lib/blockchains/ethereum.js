        ...
        wfState.getKey('blockchainKeys', privateKeyId, function ethGetKeyCb(err, privateKey) {
            ...
            function sendTransactionCheckResult(result) {
                if (!transactionCompleted) {
                    ...
                    if (transactionHash) return log.warn(_blockchainName, `Unspecified condition while sending transaction ${transactionHash}: ${JSON.stringify(result)}`);
                    return log.warn(_blockchainName, `Unspecified condition while sending raw transaction: ${JSON.stringify(result)}`);
                }
                // Otherwise it is unclear
