...
function initEthereum(bcConfig, callback) {
    ...
    wfState.getBlockchainData(_blockchainName, function blockchainsGetStateDb(err, blockchainState) {
        ...
        .then(function web3GetBlockNumberResolve(highestBlock) {
            ...
            if (_currentBlock < 1 && bcConfig.blockRetrievalStart < 1) {
                if (_blockchainState.status.currentBlock > 0) {
                    if ((highestBlock - _blockchainState.status.currentBlock) > _blockRetrievalRestart) {
                        _currentBlock = highestBlock - _blockRetrievalRestart - 1;
                        let nextBlock = _currentBlock + 1;
                    ...
                    if ((highestBlock - _blockchainState.status.currentBlock) < _blockRetrievalRestart) {
                        ...
                        if (arrearBlocks < 0) {
                            ...
                            log.info(_blockchainName, `Resuming from current block: ${_currentBlock} (${arrearBlocks} blocks behind the highest block ${highestBlock} on node)`);
                        }
                    }
                } else {
                    _currentBlock = highestBlock - 1;
                    log.info(_blockchainName, `Starting from current highest block on node: ${highestBlock}`);
                }
            }
            ...
            return callback(new Error(`Cannot initialise transaction listener: ${err.message}`), _blockchainName);
        });
    });
}
...
function getBlockchainData() {
    ...
    .then(function web3GetBlockNumberResolve(highestBlock) {
        ...
        if (_currentBlock > highestBlock) {
            log.debug(_blockchainName, `Current block ${_currentBlock} is higher than highest block ${highestBlock} on node`);
            return scheduleBlockchainData();
        }
        ...
        if (_blockRetrievalEnd > 0 && _blockRetrievalEnd < highestBlock) {
            ...
            log.info(_blockchainName, `Last block to be retrieved is set to ending block in configuration: ${endBlock}`);
        }
        // Process blocks sequentially while counting iterations
        _blockStackSize = 0;
        getBlocks(_currentBlock, endBlock, function ethGetBlocksCb(err) {
            if (err) {
            ...
            if (_currentBlock === _blockRetrievalEnd && _blockRetrievalEnd < highestBlock) {
                log.info(_blockchainName, `Reached configured block retrieval end: ${_currentBlock}`);
        ...
        log.warn(_blockchainName, `Cannot determine highest block on node: ${err.message}`);
        return scheduleBlockchainData();
 ...
 * @private
 * @param {number} currentBlock the block after which the next blocks are processed
 ...
 */
function getBlocks(currentBlock, endBlock, callback) {
    ...
    // Block to be processed is the next block after the latest (current) block
    let thisBlock = currentBlock + 1;

    // Skip this block if the block has been retried too often
    if (_blockMaxRetries > 0 && _blockRetryCount > _blockMaxRetries) {
        // Skip blocks
        _skippedBlocks += 1;
        thisBlock += _skippedBlocks;
        log.warn(_blockchainName, `Skipping block ${(thisBlock - 1)} after ${(_blockRetryCount - 1)} retries`);
    ...
    function nextBlock() {
        ...
        if (thisBlock < endBlock) {
            return getBlocks(thisBlock, endBlock, function getBlocksCb(err) {
                if (err) return callback(err);
                return callback(null);
            });
        } else {
