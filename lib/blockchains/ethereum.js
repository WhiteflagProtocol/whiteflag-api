...
function initEthereum(bcConfig, callback) {
    ...
    wfState.getBlockchainData(_blockchainName, function blockchainsGetStateDb(err, blockchainState) {
        ...
        log.debug(_blockchainName, `Using Web3 version ${_web3.version}`);
        ...
        .then(function web3GetBlockNumberResolve(highestBlock) {
            ...
            return callback(new Error(`Could not connect to Ethereum node: ${err.message}`), _blockchainName);
        });
    });
}
...
function updateNodeInfo() {
    ...
    timeoutPromise(_web3.eth.getNodeInfo(), _rpcTimeout).then(nodeInfo => {
        _blockchainState.parameters.nodeInfo = nodeInfo;
    }).catch(err => {
        log.warn(_blockchainName, `Could not get information from node: ${err.message}`);
    });
    // Get protocol version
    timeoutPromise(_web3.eth.getProtocolVersion(), _rpcTimeout).then(protocolVersion => {
        _blockchainState.parameters.protocolVersion = protocolVersion;
    }).catch(err => {
        log.warn(_blockchainName, `Could not get protocol version from node: ${err.message}`);
    });
    // Get network ID
    timeoutPromise(_web3.eth.net.getId(), _rpcTimeout).then(networkID => {
        _blockchainState.parameters.networkID = networkID;
    }).catch(err => {
        log.warn(_blockchainName, `Could not get Network ID from node: ${err.message}`);
    });
    // Check chain ID
    timeoutPromise(_web3.eth.getChainId(), _rpcTimeout).then(chainID => {
        if (chainID !== _chainID) {
        ...
        log.warn(_blockchainName, `Could not get Chain ID from node: ${err.message}`);
    });
...
function updateNodeStatus() {
    ...
    // Get peer status
    timeoutPromise(_web3.eth.net.getPeerCount(), _rpcTimeout).then(status => {
        _blockchainState.status.peers = status;
    }).catch(err => {
        log.warn(_blockchainName, `Could not get peer status: ${err.message}`);
    });
    // Get synchronisation status
    timeoutPromise(_web3.eth.isSyncing(), _rpcTimeout).then(status => {
        _blockchainState.status.syncing = status;
    }).catch(err => {
        log.warn(_blockchainName, `Could not get synchronisation status: ${err.message}`);
    });
    // Get gas price
    timeoutPromise(_web3.eth.getGasPrice(), _rpcTimeout).then(status => {
        _blockchainState.status.gasPrice = status;
    }).catch(err => {
        log.warn(_blockchainName, `Could not get gas price: ${err.message}`);
    });
}
