            ...
            if (_currentBlock < 1) {
                if (_blockchainState.status.currentBlock > 0) {
                    if ((highestBlock - _blockchainState.status.currentBlock) > _blockRetrievalRestart) {
                        ...
                        log.info(_blockchainName, `Resuming from block: ${_currentBlock} (${(highestBlock - _currentBlock)} blocks behind the highest block ${highestBlock})`);
                    }
                    if ((highestBlock - _blockchainState.status.currentBlock) < _blockRetrievalRestart) {
                        _currentBlock = _blockchainState.status.currentBlock;
                        let arrearBlocks = highestBlock - _currentBlock;
                        if (arrearBlocks < 0) {
                            log.info(_blockchainName, `Resuming from current block: ${_currentBlock} (when node catches up from its current highest block ${highestBlock})`);
                        } else {
                            log.info(_blockchainName, `Resuming from current block: ${_currentBlock} (${arrearBlocks} blocks behind the highest block ${highestBlock})`);
                        }
                    }
                } else {
...
function deleteAccount(address, callback) {
    ...
    if (index < 0) {
        return callback(new ProcessingError(`Cannot find ${_blockchainName} account: ${address}`, null, 'WF_API_NO_RESOURCE'));
    }
    ...
    if (account.address === address) {
        ...
        return callback(new Error(`Cannot not delete account: ${address}`));
    }
    // Log and return result
...
function getBlockchainData() {
    ...
    .then(function web3GetBlockNumberResolve(highestBlock) {
        ...
        if (_currentBlock > highestBlock) {
            log.debug(_blockchainName, `Current block ${_currentBlock} is higher than highest block ${highestBlock}`);
            return scheduleBlockchainData();
        }
        ...
        getBlocks(_currentBlock, lastBlock, function ethGetBlocksCb(err) {
            ...
            if (_currentBlock === _blockRetrievalEnd && _blockRetrievalEnd < highestBlock) {
                ...
                if ((highestBlock - _currentBlock) > _blockRetrievalRestart) {
                    _currentBlock = highestBlock - _blockRetrievalRestart;
                    log.info(_blockchainName, `Resuming from block: ${_currentBlock} (${(highestBlock - _currentBlock)} blocks behind highest block ${highestBlock})`);
                }
                if ((highestBlock - _currentBlock) < _blockRetrievalRestart) {
