        ...
        wfState.getKey('blockchainKeys', privateKeyId, function ethGetKeyCb(err, privateKey) {
            ...
            function sendTransactionConfirmation(confNumber, receipt) {
                if (!transactionConfirmation) {
                    ...
                    if (confNumber > 1 && transactionHash) {
                        log.warn(_blockchainName, `Still trying to get receipt for transaction ${transactionHash} after ${confNumber} confirmations`);
                        transactionConfirmation = true;
            ...
            function sendTransactionGetReceipt() {
                if (!transactionCompleted) {
                    ...
                    let receipt = _web3.eth.getTransactionReceipt(ethFormatHex(transactionHash));

                    // Check receipt and numbe rof retires
                    if (!receipt && receiptRetryCount > MAXRETRIES) {
