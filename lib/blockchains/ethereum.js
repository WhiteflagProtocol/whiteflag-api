 ...
 * @param {bcInitCb} callback function to be called after intitialising Ethereum
 */
 ...
 * @param {string} ethPublicKey the raw hex public key of the originator
 * @param {bcRequestKeysCb} callback function to be called upon completion
 */
function requestKeys(ethPublicKey, callback) {
    log.trace(MODULELOG, `Getting address and encoded keys for Ethereum public key: ${ethPublicKey}`);
    ...
    try {
        ...
        const publicKeyHex = noPubkeyHexPrefix(ethPublicKey);
        const publicKeyBuffer = hexToBuffer(publicKeyHex);
        originatorKeys.publicKey.hex = publicKeyHex;
        originatorKeys.publicKey.pem = keyEncoder.encodePublic(publicKeyHex, 'raw', 'pem');

        // Keccak double check
        let keccakPubkeyBuffer;
        if (publicKeyHex.length === 130) {
            keccakPubkeyBuffer = hexToBuffer(publicKeyHex.substring(2));
        } else {
            keccakPubkeyBuffer = publicKeyBuffer;
        }
...
function requestSignature(wfSignPayload, callback) {
    ...
    .then(account => {
        ...
        wfState.getKey('blockchainKeys', privateKeyId, function ethGetKeyCb(err, ethPrivateKey) {
            ...
            let wfSignature;
            try {
                wfSignature = createJWS(wfSignPayload, noKeyHexPrefix(ethPrivateKey), SIGNKEYTYPE, SIGNALGORITHM);
            } catch(err) {
 ...
 * @param {wfSignature} wfSignDecoded the full JWS with the Whiteflag signature
 ...
 */
function verifySignature(wfSignDecoded, ethPublicKey, callback) {
    log.trace(MODULELOG, `Validating signature: ${JSON.stringify(wfSignature)}`);
    let result;
    try {
        result = verifyJWS(wfSignDecoded, ethPublicKey, SIGNKEYTYPE);
    } catch(err) {
        log.error(MODULELOG, `Could not not verify signature: ${err.message}`);
        return callback(err);
    }
    //TODO: Check public key and address
    return callback(null, result, ethPublicKey);
}
...
function getBinaryAddress(ethAddress, callback) {
    ...
    return callback(new ProcessingError(`Invalid ${_ethChain} address: ${ethAddress}`, null, 'WF_API_PROCESSING_ERROR'));
}
