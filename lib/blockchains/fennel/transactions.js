...
function initTransactions(fnlConfig, fnlState) {
    ...
    log.trace(_fnlName, 'Initialising Fennel transaction processing...');
...
function sendTransaction(account, toAddress, value, data) {
    log.trace(_fnlName, `Sending transaction from account: ${account.address}`);
    return new Promise((resolve, reject) => {
        // Create transaction
        createTransaction(account, toAddress, value, data)
        .then(fnlRawExtrinsic => {
            // Get the private key to sign the transaction
            const privateKeyId = hash(_fnlName + noAddressHexPrefix(account.address), KEYIDLENGTH);
...
function getTransaction(transactionHash) {
    log.trace(_fnlName, `Retrieving transaction: ${transactionHash}`);
 ...
 */
 function extractMessage(transaction, timestamp) {
    if (_traceRaw) log.trace(_fnlName, `Extracting Whiteflag message from transaction`);
