...
function initRpc(fnlConfig, fnlState) {
    ...
    log.info(_fnlName, `Timeout for remote calls to the Fennel node: ${_rpcTimeout} ms`);

    // Get Node URL and credentials
    _fnlState.parameters.rpcURL = getFullURL(fnlConfig, true);    // no credentials
    _rpcAuthURL = getFullURL(fnlConfig, false);                      // include credentials
    ...
    return new Promise((resolve, reject) => {
        switch (fnlConfig.rpcProtocol) {
            ...
            case 'ws':
            case 'wss': {
                log.trace(_fnlName, `Initialising Fennel web socket RPC connection with: ${_fnlState.parameters.rpcURL}`);
                _websocket = true;
                ...
                .then(api => {
                    ...
                    _fnlApi = api;
                    log.info(_fnlName, `Connected to ${_fnlApi.runtimeVersion.specName} through node: ${_fnlState.parameters.rpcURL}`);
                ...
                .catch(err => {
                    ...
                    return reject(new Error(`Could not make a web socket connection with Fennel node: ${err.message}`), _fnlName);
                });
                break;
            }
            // Connect via http
            case 'http':
            case 'https': {
                log.trace(_fnlName, `Initialising Fennel http RPC connection with: ${_fnlState.parameters.rpcURL}`);
                
                getRuntimeVersion()
                .then(fnlRuntime => {
                    // We have a connection
                    log.info(_fnlName, `Connected to ${fnlRuntime.specName} through node: ${_fnlState.parameters.rpcURL}`);
                ...
                .catch(err => reject(new Error(`Could not make web connection with Fennel node: ${err.message}`), _fnlName));
                break;
            }
            // Unknown protocol
            default: {
                return reject(new Error(`Unknown protocol to connect with Fennel node: ${fnlConfig.rpcProtocol}`), _fnlName);
            }
        }
    });
}
...
function getBalance(address) {
    ignore(address);
    return Promise.reject(new ProcessingError('No method implemented to get balance for an account' ,null , 'WF_API_NOT_IMPLEMENTED'));
}
...
function getTransactionCount(address) {
    ignore(address);
    return Promise.reject(new ProcessingError('No method implemented to get transaction count for an account' ,null , 'WF_API_NOT_IMPLEMENTED'));
}
...
async function updateNodeInfo() {
    ...
    .catch(err => log.warn(_fnlName, `Could not get system name from node: ${err.message}`));

    // Get system version info
    await getSystemVersion()
    .then(systemVersion => (_fnlState.parameters.systemVersion = systemVersion  || ''))
    .catch(err => log.warn(_fnlName, `Could not get system version from node: ${err.message}`));
    ...
    .catch(err => log.warn(_fnlName, `Could not get chain runtime information from node: ${err.message}`));

    // Get node roles
    await getChainType()
    .then(chainType => (_fnlState.parameters.chainType = chainType || ''))
    .catch(err => log.warn(_fnlName, `Could not get chain type from node: ${err.message}`));

    // Get node roles
    await getNodeRoles()
    .then(nodeRoles => (_fnlState.parameters.nodeRoles = nodeRoles || ''))
    .catch(err => log.warn(_fnlName, `Could not get roles from node: ${err.message}`));
}
...
async function updateNodeStatus() {
    ...
    .catch(err => log.warn(_fnlName, `Could not get peer status: ${err.message}`));

    // Get synchronisation status
    await isSyncing()
    .then(syncing => (_fnlState.status.syncing = syncing  || ''))
    .catch(err => log.warn(_fnlName, `Could not get synchronisation status: ${err.message}`));
...
function saveNodeStatus() {
    ...
    log.info(_fnlName, `Status: {${JSON.stringify(_fnlState.parameters)}, ${JSON.stringify(_fnlState.status)}}`);
}
