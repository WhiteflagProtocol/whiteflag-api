...
function initRpc(fennelConfig, fennelState) {
    ...
    // Check for authorization and configure provider
    if (fennelConfig.username && fennelConfig.username.length > 0
     && fennelConfig.password && fennelConfig.password.length > 0) {
        const rpcAuthHeader = 'Basic ' + Buffer.from(fennelConfig.username + ':' + fennelConfig.password).toString('base64');
        log.trace(_blockchainName, `Connecting to Fennel node using basic authorization: ${rpcURL}`);
        _fennelNode = new WsProvider(rpcURL, false, { Authorization: rpcAuthHeader }, _rpcTimeout);
    } else {
        log.trace(_blockchainName, `Connecting to Fennel node without authorization: ${rpcURL}`);
        _fennelNode = new WsProvider(rpcURL, false, {}, _rpcTimeout);
    }
    _fennelNode.on('error', (err) => {
        log.error(_blockchainName, `Connection error with Fennel node: ${err.message}`);
    });
    // Promise to create api
    return timeoutPromise(ApiPromise.create({
