...
const { rpcCall,
        getFullURL } = require('../_common/rpc');
...
function getBalance(address) {
    if (_fnlApi) {
        ...
        .then(({ nonce, data: balance }) => { 
            ignore(nonce);
            return Promise.resolve(Number(balance.free));
        });
    } else {
...
function sendTokens(keyring, toAddress, amount) {
    if (_fnlApi) {
        return _fnlApi.tx.balances
            .transferKeepAlive(toAddress, parseInt(amount))
            .signAndSend(keyring, { nonce: -1 });
    } else {
...
async function updateNodeStatus() {
    ...
    await getSyncState()
    .then(syncState => {
        _fnlState.status.startingBlock = Number(syncState.startingBlock) || '';
        _fnlState.status.currentBlock = Number(syncState.currentBlock) || '';
