...
function initRpc(fennelConfig, fennelState) {
    ...
    log.trace(_blockchainName, `Initialising Fennel RPC connection with: ${_fennelState.parameters.rpcURL}`);
    ...
    return new Promise((resolve, reject) => {
        ...
        if (_rpcURL.substring(0, 2) === 'ws') {
            ...
            .catch(err => reject(new Error(`Could make a web socket connection with Fennel node: ${err.message}`), _blockchainName));
        }
        // CCnnect via http
