...
function scheduleBlockIteration() {
    log.trace(MODULELOG, `Scheduling block retrieval iteration ${(_iterationCount + 1)} in ${_blockInterval} ms`);
...
async function executeBlockIteration() {
    _iterationCount += 1;
    log.trace(MODULELOG, `Starting block retrieval iteration ${_iterationCount}`);
    ...
    .then(highestBlock => {
        ...
        if (highestBlock === _blockCursor) {
            log.trace(MODULELOG, `Highest block ${highestBlock} has already been processed`);
        ...
        if (_blockCursor > highestBlock) {
            log.debug(MODULELOG, `Current block ${_blockCursor} is higher than highest block ${highestBlock} on node`);
        ...
        if ((highestBlock - 1) === _blockCursor) {
            log.trace(MODULELOG, `Waiting for new block before processing current highest block ${highestBlock}`);
            return Promise.reject(); // Stop this iteration without error
        }
    ...
    .then(async () => {
        ...
        _blockCursor = determineStartingBlock(_fnlChain, _fnlState.status.highestBlock, _blockCursor, _blockRetrievalEnd, _blockRetrievalRestart);
        logStartingBlock(_fnlChain, _blockCursor, _fnlState.status.highestBlock);
        return Promise.resolve();
    })
    .then(() => {
...
async function processBlocks(startBlock, endBlock) {
    ...
    log.trace(MODULELOG, `Retrieving block ${_blockStackSize} of iteration ${_iterationCount}: ${thisBlock}`);
    ...
    for (
        ...
        transactionBatch.push(
            new Promise((resolve, reject) => {
                fnlTransactions.extractMessage(transactions[i], blockNumber, timestamp)
                .then(wfMessage => {
