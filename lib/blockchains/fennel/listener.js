...
async function initListener(fennelConfig, fennelState) {
    ...
    log.trace(_blockchainName, 'Initialising listener for Fennel blockchain transactions...');
...
async function executeBlockIteration() {
    ...
    .then(highestBlock => {
        ...
        if (highestBlock >  _fennelState.status.highestBlock) {
            _discoveredBlock = highestBlock;
            _fennelState.status.highestBlock = highestBlock;
            log.trace(_blockchainName, `New highest block discovered on node is ${_discoveredBlock}`);
        }
        ...
        if ((highestBlock - 1) === _blockCursor) {
            log.trace(_blockchainName, `Waiting for new block before processing current highest block ${highestBlock}`);
            return Promise.reject(); // Stop this iteration without error
        }
...
async function processBlocks(startBlock, endBlock) {
    ...
    if (startBlock === endBlock) {
        log.trace(_blockchainName, `Processed block ${startBlock} of ${endBlock}; no new block to retrieve`);
        return Promise.resolve(); // Completed for now
    }
