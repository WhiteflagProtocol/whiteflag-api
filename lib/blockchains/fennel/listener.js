...
const { determineStartingBlock, logStartingBlock } = require('../_common/state');
...
async function initListener(fnlConfig, fnlState) {
    ...
    try {
        _fnlState.status.highestBlock = Number(await fnlRpc.getHighestBlock());
        _blockCursor = Number(determineStartingBlock(
            _fnlState.status.highestBlock,
            _fnlState.status.currentBlock,
            _blockRetrievalStart,
            _blockRetrievalRestart
        ));
        logStartingBlock(_fnlChain, _blockCursor, _fnlState.status.highestBlock);
    } catch(err) {
...
async function executeBlockIteration() {
    _iterationCount += 1;
    if (_traceRaw) log.trace(MODULELOG, `Starting block retrieval iteration ${_iterationCount}`);
    ...
    .then(highestBlock => {
        ...
        if (highestBlock >  _fnlState.status.highestBlock) {
            ...
            log.trace(MODULELOG, `New highest block discovered on node is ${_discoveredBlock}`);
        }
        // Check if highest block is already processed, and schedule next iteration
        if (highestBlock === _blockCursor) {
            if (_traceRaw) log.trace(MODULELOG, `Highest block ${highestBlock} has already been processed`);
        ...
        if (_blockCursor > highestBlock) {
            log.trace(MODULELOG, `Current block ${_blockCursor} is higher than highest block ${highestBlock} on node`);
        ...
        if ((highestBlock - 1) === _blockCursor) {
            if (_traceRaw) log.trace(MODULELOG, `Waiting for new block before processing current highest block ${highestBlock}`);
            return Promise.reject(); // Stop this iteration without error
        }
    ...
    .then(async () => {
        ...
        log.info(MODULELOG, `Reached configured block retrieval end: ${_blockCursor}`);
...
async function processBlocks(startBlock, endBlock) {
    ...
    if (_blockStackSize > BLOCKSTACKSIZE) {
        log.trace(MODULELOG, `Reached maximum block processing stack size: ${_blockStackSize}`);
        return Promise.resolve(); // Completed for now
    }
    // If current block is last block, then there is no new block to retrieve
    if (startBlock === endBlock) {
        log.trace(MODULELOG, `Processed block ${startBlock} of ${endBlock}; no new block to retrieve`);
        return Promise.resolve(); // Completed for now
    }
    // Block to be processed is the next block after starting block and skipped blocks
    if (_skippedBlocks > 0) log.trace(MODULELOG, `Skipped ${_skippedBlocks} blocks since block ${startBlock}`);
    ...
    if (_blockMaxRetries > 0 && _blockRetryCount > _blockMaxRetries) {
        log.warn(MODULELOG, `Skipping block ${thisBlock} after ${_blockMaxRetries} retries`);
        thisBlock += 1;
        _blockRetryCount = 0;
        _skippedBlocks += 1;
    }
    // Log where we are with blocks
    if (_blockRetryCount !== 0) log.info(MODULELOG, `Retry ${_blockRetryCount} to process block: ${thisBlock}`);
    if (_traceRaw) log.trace(MODULELOG, `Retrieving block ${_blockStackSize} of iteration ${_iterationCount}: ${thisBlock}`);
...
async function processBlock(blockNumber) {
    ...
    let block;
    try {
        block = await fnlRpc.getBlockByNumber(blockNumber, true); // Get full block including transactions
    } catch(err) {
    ...
    return processTransactions(blockNumber, block.extrinsics, block.time)
    .then(() => {
        log.info(MODULELOG, `Transactions processed from block ${blockNumber}: ${transactionCount} `);
        return Promise.resolve(); // Completed this block
    })
    .catch(err => {
 ...
 */
function processTransactions(blockNumber, transactions, timestamp) {
    // Get transaction batch of Promises in an array to extract Whiteflag messages
    const transactionBatch = createTransactionBatch(blockNumber, transactions, timestamp);
...
/**
 * Combines all transactions from a Fennel block as promises in an array for batch processing
 * @private
 * @param {number} blockNumber the block number of the transactions
 * @param {Array} transactions the transactions to process
 * @param {number} timestamp the block timestamp
 ...
 */
function createTransactionBatch(blockNumber, transactions, timestamp) {
    return transactions.map(transaction => {
        return new Promise((resolve, reject) => {
            fnlTransactions.extractMessage(transaction, blockNumber, timestamp)
            .then(wfMessage => {
            ...
            .catch(err => {
                ...
                return reject(new Error(`Could not process transaction ${transaction.hash}: ${err.message}`)); // Other error
            });
        });
    });
}
