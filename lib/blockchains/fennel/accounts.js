 ...
 * @return {Promise} resolves to the signature
 */
    ...
    return new Promise((resolve, reject) => {
        ...
        .then(account => {
            ...
            wfState.getKey('blockchainKeys', privateKeyId, function fnlGetKeyCb(err, privateKey) {
                if (err) reject(err);
                const keypair = {
                    publicKey: hexToBuffer(account.publicKey),
                    secretKey: hexToBuffer(privateKey)
                }
...
/**
 * Encodes Uint8Arrays into hexadecimal string
 * @param {Uint8Array} u8array the encoding used by Polkadot
