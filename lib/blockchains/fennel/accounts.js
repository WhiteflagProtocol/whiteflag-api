...
function getAddress(fnlPublicKey) {
    return new Promise((resolve, reject) => {
        let fnlAddress;
        try {
            fnlAddress = fnlCrypto.encodeAddress(hexToU8a(fnlPublicKey));
        } catch(err) {
            return reject(err);
        }
        return resolve(fnlAddress);
    });
}
 ...
 * @alias module:lib/blockchains/fennel.createKeypair
...
function getSignature(input, address) {
    log.trace(MODULELOG, `Signing input for address ${address}: ${input}`);
    return new Promise((resolve, reject) => {
        getAccount(address)
        .then(account => {
            ...
            wfState.getKey('blockchainKeys', privateKeyId, function fnlGetKeyCb(err, fnlPrivateKey) {
                ...
                log.trace(MODULELOG, `Created signature for address ${address}: ${signature}`);
...
function verifySignature(input, signature, publicKey) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            return reject(new ProcessingError(`Error verifying signature: ${err.message}`, null, 'WF_SIGN_ERROR'));
        }
    });
}
...
function testSignature(input, signature, publicKey) {
    return fnlCrypto.sr25519Verify(input, hexToU8a(signature), hexToU8a(publicKey));
}
