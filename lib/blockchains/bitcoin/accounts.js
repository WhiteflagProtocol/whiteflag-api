...
module.exports = {
    ...
    updateUtxos: updateAccountUtxos,
...
/**
 * Updates the status of the UXTOs for an account
 ...
 * @param {Array} utxoInputList the input UXTOs of a transaction
 * @param {boolean} confirmed true if the transaction is confirmed
 * @returns {Object} the updated account
 */
 function updateAccountUtxos(account, utxoInputList, confirmed) {
    // Is the transaction confirmed?
    let spent = UTXOSTATUS.NEEDSVERIFICATION;
    if (confirmed) spent = UTXOSTATUS.SPENT;

    // Check account UXTOs against the list of input UXTOs
...
function processBlockUtxos(blockNumber, block) {
    for (let account of _bcState.accounts) {
        ...
        if (account.lastBlock === (blockNumber - 1)) {
            ...
            .catch(err => {
                log.warn(_blockchainName, `Error processing block ${blockNumber} to check UXTOs for account ${account.address}: ${err.message}`);
...
function synchroniseAccount(account) {
    ...
    if (account.lastBlock >= _bcState.status.currentBlock) {
        if (account.syncing) {
            log.info(_blockchainName, `Completed synchronisation of account ${account.address} at block: ${account.lastBlock}/${_bcState.status.currentBlock}`);
            account.syncing = false;
        }
        return Promise.resolve();
    }
    if (!account.syncing) {
        log.info(_blockchainName, `Starting synchronisation of account ${account.address} from block: ${account.lastBlock}/${_bcState.status.currentBlock}`);
        account.syncing = true;
    }
    // Get next block
    let thisBlock = account.lastBlock + 1;
    log.trace(_blockchainName, `Synchronising account ${account.address} with block: ${thisBlock}/${_bcState.status.currentBlock}`);
 ...
 function processTransaction(transaction, account) {
    for (let indexOut of Object.keys(transaction.vout)) {
        if (typeof transaction.vout[indexOut].scriptPubKey.addresses !== 'undefined') {
            checkAccountUtxosReceived(transaction, account, indexOut);
        }
    }
    for (let indexIn of Object.keys(transaction.vin)) {
        if (typeof transaction.vin[indexIn].txid !== 'undefined') {
            for (let otherAccount of _bcState.accounts) {
                checkAccountUtxosSpent(transaction, otherAccount, indexIn);
            }
        }
    }
}
 ...
 */
function checkAccountUtxosReceived(transaction, account, index) {
    if (account.address === transaction.vout[index].scriptPubKey.addresses[0]) {
        if (!account.utxos.some(utxo => utxo.txid === transaction.txid)) {
            ...
            log.info(_blockchainName, 'Received ' + (transaction.vout[index].value * SATOSHI).toFixed(0) + 'satoshis on Bitcoin account: ' + account.address);
 ...
 * @param {Object} index index of the transaction
 */
function checkAccountUtxosSpent(transaction, account, index) {
    for (let utxo of account.utxos) {
