    ...
    return new Promise((resolve, reject) => {
        ...
        .then(account => {
            if (account.syncing === true) {
                return reject(new ProcessingError(`The ${_blockchainName} account cannot be used because it is currently syncing at block: ${account.lastBlock}/${_btcState.status.currentBlock}`, null, 'WF_API_NOT_AVAILABLE'));
            }
            resolve(account);
        })
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            if (wif !== null) {
                ...
                account = generateAccount(bitcoin.ECPair.fromWif(wif, _btcState.parameters.network));
            } else {
                log.trace(_blockchainName, 'Creating new Bitcoin account with generated keys');
                account = generateAccount(bitcoin.ECPair.makeRandom({ network: _btcState.parameters.network }));
...
function synchroniseAccount(account) {
    ...
    if (account.lastBlock >= _btcState.status.currentBlock) {
        if (account.syncing) {
            log.info(_blockchainName, `Completed synchronisation of account ${account.address} at block: ${account.lastBlock}/${_btcState.status.highestBlock}`);
            account.syncing = false;
        }
        return Promise.resolve();
    }
    if (!account.syncing) {
        log.info(_blockchainName, `Starting synchronisation of account ${account.address} from block: ${account.lastBlock}/${_btcState.status.highestBlock}`);
        account.syncing = true;
    }
    // Get next block
    let thisBlock = account.lastBlock + 1;
    log.trace(_blockchainName, `Synchronising account ${account.address} with block: ${thisBlock}/${_btcState.status.highestBlock}`);
