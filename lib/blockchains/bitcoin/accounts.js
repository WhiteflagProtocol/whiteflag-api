 ...
 * @todo Move key pair creation to createAccountEntry() for testability
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            if (wif !== null) {
                ...
                account = createAccountEntry(ECPair.fromWIF(wif, _btcState.parameters.network));
            } else {
                log.trace(MODULELOG, `Creating new ${_btcChain} account with generated keys`);
                account = createAccountEntry(ECPair.makeRandom({ network: _btcState.parameters.network }));

                // New account should not have transactions before the current highest block
                btcRpc.getBlockCount()
                .then(highestBlock => {
                    account.firstBlock = highestBlock || 1;
                    account.lastBlock = highestBlock || 1;
                });
            }
 ...
 * @param {bcDeleteAccountCb} callback function to be called upon completion
 */
    ...
    for (let account of _btcState.accounts) {
        ...
        if (account.lastBlock === (blockNumber - 1)) {
            ...
            .catch(err => {
                ...
                return scheduleSynchroniseAccount(account.address, true);
            });
        } else {
...
function createAccountEntry(bcKeypair) {
    return {
        address: payments.p2pkh({
            pubkey: Buffer.from(bcKeypair.publicKey),
            network: _btcState.parameters.network
        }).address,
        publicKey: Buffer.from(bcKeypair.publicKey).toString(HEXENCODING),
        privateKey: Buffer.from(bcKeypair.privateKey).toString(HEXENCODING),
        balance: 0,
        firstBlock: 1,
        lastBlock: 0,
        syncing: false,
 ...
 * @private
 */
function scheduleSynchroniseAccount(address, delay = false) {
    getAccount(address)
    .then(account => {
        setTimeout(function timeoutSynchroniseAccountCb() {
            synchroniseAccount(account);
        }, delay ? SYNCTIMER : SYNCDELAY);
    })
    .catch(err => {
        log.warn(MODULELOG, `Cannot synchronise account ${address}: ${err.message}`);
    });
}
...
function synchroniseAccount(account) {
    ...
    .catch(err => {
        log.warn(MODULELOG, `Rescheduling synchronisation of account ${account.address} after failure to process block ${thisBlock}: ${err.message}`);
        return scheduleSynchroniseAccount(account.address, true);
    });
}
...
function checkAccountUtxosReceived(transaction, index, account) {
    if (account.address === transaction.vout[index].scriptPubKey.addresses[0]) {
        if (!account.utxos.some(utxo => utxo.txid === transaction.txid)) {
            ...
            log.info(MODULELOG, `Received ${utxo.value} satoshis on account: ${account.address}`);
...
function checkAccountUtxosSpent(transaction, index, account) {
    for (let utxo of account.utxos) {
        if (utxo.txid === transaction.vin[index].txid
            ...
            log.info(MODULELOG, `Spent ${utxo.value} satoshis from account: ${account.address}`);
