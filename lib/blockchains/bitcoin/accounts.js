...
/**
 * Sets required parameters for Bitcoin accounts module
 * @param {*} blockchainState
 * @param {*} network
 */
function setParameters(blockchainState, network) {
    _network = network;
 ...
 * @TODO Resolve unused callback
 */
function createAccount(wif) {
...
function getAccount(address) {
    return _blockchainState.accounts.find(account => {
        if (account.address === address) {
            return account;
        }
    });
...
function generateNewAccount() {
    ...
    let account = generateAccount(keyPair);
...
function generateAccountFromWif (wif) {
    ...
    return generateAccount(keyPair);
}
