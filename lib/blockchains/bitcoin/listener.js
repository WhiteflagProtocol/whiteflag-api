...
async function executeBlockIteration() {
    ...
    .then(async () => {
        ...
        _blockCursor = blockchainUtil.determineStartingBlock(_blockchainName, _bcState.status.highestBlock, _blockCursor, _blockRetrievalEnd, _blockRetrievalRestart);
        blockchainUtil.logStartingBlock(_blockchainName, _blockCursor, _bcState.status.highestBlock);
        return Promise.resolve();
    })
    .then(() => {
