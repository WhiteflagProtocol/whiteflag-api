...
async function initListener(btcConfig, btcState) {
    log.trace(_btcName, 'Initialising listener for Bitcoin blockchain transactions...');
    _btcName = btcConfig.name;
    _btcState = btcState;

    // Block processing parameters
    if (btcConfig.blockMaxRetries) _blockMaxRetries = btcConfig.blockMaxRetries;
    if (_blockMaxRetries > 0) log.debug(_btcName, `Maximum retries for processing a block is set to ${_blockMaxRetries} for each block`);
    if (btcConfig.transactionBatchSize) _transactionBatchSize = btcConfig.transactionBatchSize;
    log.debug(_btcName, `Maximum number of transactions in a block that are processed in parallel is set to ${_transactionBatchSize}`);

    // Block interval time
    if (btcConfig.blockRetrievalInterval && btcConfig.blockRetrievalInterval > 500) {
        _blockInterval = btcConfig.blockRetrievalInterval;
    }
    log.info(_btcName, `Block retrieval interval is set to ${_blockInterval} ms`);
    ...
    if (btcConfig.blockRetrievalStart && btcConfig.blockRetrievalStart > 0) {
        log.info(_btcName, `Starting block specified in configuration: ${btcConfig.blockRetrievalStart}`);
        _blockRetrievalStart = btcConfig.blockRetrievalStart;
    }
    if (btcConfig.blockRetrievalEnd && btcConfig.blockRetrievalEnd > 0) {
        log.info(_btcName, `Ending block specified in configuration: ${btcConfig.blockRetrievalEnd}`);
    ...
    try {
        ...
        logStartingBlock(_btcName, _blockCursor, _btcState.status.highestBlock);
    } catch(err) {
...
function scheduleBlockIteration() {
    log.trace(_btcName, `Scheduling block retrieval iteration ${(_iterationCount + 1)} in ${_blockInterval} ms`);
...
async function executeBlockIteration() {
    _iterationCount += 1;
    log.trace(_btcName, `Starting block retrieval iteration ${_iterationCount}`);
    ...
    .then(highestBlock => {
        ...
        if (highestBlock >  _discoveredBlock) {
            ...
            log.trace(_btcName, `New highest block discovered on node is ${_discoveredBlock}`);
        }
        // Check if highest block is already processed, and schedule next iteration
        if (highestBlock === _blockCursor) {
            log.trace(_btcName, `Highest block ${highestBlock} has already been processed`);
        ...
        if (_blockCursor > highestBlock) {
            log.debug(_btcName, `Current block ${_blockCursor} is higher than highest block ${highestBlock} on node`);
    ...
    .then(async () => {
        ...
        log.info(_btcName, `Reached configured block retrieval end: ${_blockCursor}`);
        _blockRetrievalEnd = 0;

        // Dtermine from where to proceed
        _blockCursor = determineStartingBlock(_btcName, _btcState.status.highestBlock, _blockCursor, _blockRetrievalEnd, _blockRetrievalRestart);
        logStartingBlock(_btcName, _blockCursor, _btcState.status.highestBlock);
        return Promise.resolve();
    })
    .then(() => {
    ...
    .catch(err => {
        if (err) {
            ...
            log.warn(_btcName, `Could not complete retrieval of block ${_blockCursor} in iteration ${_iterationCount}: ${err.message}`);
...
async function processBlocks(startBlock, endBlock) {
    ...
    if (_blockStackSize > BLOCKSTACKSIZE) {
        log.trace(_btcName, `Reached maximum block processing stack size: ${_blockStackSize}`);
        return Promise.resolve(); // Completed for now
    }
    // If current block is last block, then there is no new block to retrieve
    if (startBlock === endBlock) {
        log.trace(_btcName, 'No new block to retrieve');
        return Promise.resolve(); // Completed for now
    }
    // Block to be processed is the next block after starting block and skipped blocks
    if (_skippedBlocks > 0) log.trace(_btcName, `Skipped ${_skippedBlocks} blocks since block ${startBlock}`);
    ...
    if (_blockMaxRetries > 0 && _blockRetryCount > _blockMaxRetries) {
        log.warn(_btcName, `Skipping block ${thisBlock} after ${_blockMaxRetries} retries`);
        thisBlock += 1;
        _blockRetryCount = 0;
        _skippedBlocks += 1;
    }
    // Log where we are with blocks
    if (_blockRetryCount !== 0) log.info(_btcName, `Retry ${_blockRetryCount} to process block: ${thisBlock}`);
    log.trace(_btcName, `Retrieving block ${_blockStackSize} of iteration ${_iterationCount}: ${thisBlock}`);
...
async function processBlock(blockNumber) {
    ...
    if (!block || transactionCount < 1) {
        ...
        log.info(_btcName, `No transactions in block: ${blockNumber}`);
        return Promise.resolve(); // Completed this block
    }
    log.trace(_btcName, `Transactions discovered in block ${blockNumber}: ${transactionCount}`);
    ...
    .then(() => {
        log.info(_btcName, `Transactions processed from block ${blockNumber}: ${transactionCount} `);
        return Promise.resolve(); // Completed this block
    })
    .catch(err => {
    ...
    for (
        ...
        transactionBatch.push(
            new Promise((resolve, reject) => {
                ...
                .then(wfMessage => {
                    log.trace(_btcName, `Received Whiteflag message: ${JSON.stringify(wfMessage.MetaHeader)}`);
