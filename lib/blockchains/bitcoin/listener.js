...
async function initListener(btcConfig, btcState) {
    ...
    if (Object.hasOwn(btcConfig, 'blockMaxRetries')) _blockMaxRetries = btcConfig.blockMaxRetries;
    ...
    // Block interval time
    if (Object.hasOwn(btcConfig, 'blockRetrievalInterval') && btcConfig.blockRetrievalInterval > 500) {
    ...
    if (Object.hasOwn(btcConfig, 'blockRetrievalRestart')) {
        log.debug(MODULELOG, `Number of blocks to look back specified in configuration: ${fnlConfig.blockRetrievalRestart}`);
        _blockRetrievalRestart = btcConfig.blockRetrievalRestart;
    }
    if (Object.hasOwn(btcConfig, 'blockRetrievalStart') && btcConfig.blockRetrievalStart > 0) {
    ...
    if (Object.hasOwn(btcConfig, 'blockRetrievalEnd') && btcConfig.blockRetrievalEnd > 0) {
