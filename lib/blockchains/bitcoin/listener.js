...
async function initListener(btcConfig, btcState) {
    ...
    try {
        ...
        logStartingBlock(_btcChain, _blockCursor, _btcState.status.highestBlock);
    } catch(err) {
...
async function executeBlockIteration() {
    ...
    .then(async () => {
        ...
        _blockCursor = determineStartingBlock(_btcChain, _btcState.status.highestBlock, _blockCursor, _blockRetrievalEnd, _blockRetrievalRestart);
        logStartingBlock(_btcChain, _blockCursor, _btcState.status.highestBlock);
        return Promise.resolve();
    })
    .then(() => {
...
async function processBlocks(startBlock, endBlock) {
    ...
    if (startBlock === endBlock) {
        log.trace(MODULELOG, 'No new block to retrieve');
        return Promise.resolve(); // Completed for now
    }
