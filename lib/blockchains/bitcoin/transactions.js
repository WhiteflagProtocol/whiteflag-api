...
function initTransactions(btcConfig, btcState) {
    ...
    _btcState = btcState;
    log.trace(_btcName, 'Initialising Bitcoin transaction processing...');
...
function sendTransaction(account, toAddress, amount, data = null) {
    log.trace(_btcName, `Sending transaction from account: ${account.address}`);
    return new Promise((resolve, reject) => {
        ...
        .then(signedTransaction => {
            log.trace(_btcName, `Sending signed transaction: ${JSON.stringify(signedTransaction)}`);
...
function getTransaction(transactionHash) {
    log.trace(_btcName, `Retrieving transaction: ${transactionHash}`);
    return new Promise((resolve, reject) => {
...
function extractMessage(transaction, blockNumber, timestamp) {
    if (_traceRaw) log.trace(_btcName, `Extracting Whiteflag message from transaction: ${transaction.hash}`);
    return new Promise((resolve, reject) => {
 ...
 */
 function calculateTransactionFee(transactionSize) {
    ...
    .catch(err => {
        ...
        log.warn(_btcName, `Could not determine actual transaction fee rate: ${err.message}`);
