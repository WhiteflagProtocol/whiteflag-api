...
function sendTransaction(account, toAddress, amount, data = null) {
    log.trace(MODULELOG, `Sending transaction from ${_btcChain} account: ${account.address}`);
    return new Promise((resolve, reject) => {
        ...
        .then(signedTransaction => {
            log.trace(MODULELOG, `Sending signed transaction: ${JSON.stringify(signedTransaction)}`);
...
function extractMessage(transaction, blockNumber, timestamp) {
    if (_traceRaw) log.trace(MODULELOG, `Extracting Whiteflag message from ${_btcChain} transaction: ${transaction.hash}`);
    return new Promise((resolve, reject) => {
        ...
        return reject(new ProcessingError(`No Whiteflag message data found in ${_btcChain} transaction: ${transaction.hash}`, null, 'WF_API_NO_DATA'));
    });
}
 ...
 function signTransaction(account, transaction, btcNetwork) {
    return new Promise((resolve, reject) => {
        ...
        wfState.getKey('blockchainKeys', privateKeyId, function bcGetKeyCb(err, privateKey) {
            ...
            btcAccounts.createKeypair(privateKey, btcNetwork)
            .then(keyPair => {
 ...
 */
 function calculateTransactionFee(transactionSize) {
    ...
    .catch(err => {
        ...
        log.warn(MODULELOG, `Could not determine actual transaction fee rate: ${err.message}`);
