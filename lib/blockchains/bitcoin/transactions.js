...
function sendTransaction(account, toAddress, amount, data = null) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let transaction;
        try {
            ({ utxoInputList, transaction } = createTransaction(account, toAddress, amount, data));
        } catch(err) {
 ...
 */
 function createTransaction(account, toAddress, amount, data = null) {
    // New transactions
    ...
    const transactionSize = (utxoInputList.length * 146) + (nOutputs * 33) + 10;
    const transactionFee = getTransactionFee((transactionSize));
    const returnValue = (totalUtxoValue - (amount + transactionFee));
    transaction.addOutput(account.address, returnValue);

    // Return the new transaction
    log.trace(_blockchainName, `Created a new Bitcoin transaction with a fee of ${transactionFee} satoshis`);
    return {
        utxoInputList,
        transaction
    };
...
function getTransactionFee(transactionSize) {
    // Use fixed transaction fee if priority not set
    if (!_transactionPriority) return _transactionFee;
    bcRpc.getFeeRate(_transactionPriority)
    .then(estimate => {
        if (!estimate.feerate) return _transactionFee;
        return Math.ceil((estimate.feerate * SATOSHI) * (transactionSize / KILOBYTE));
    })
    .catch((err) => {
        log.warn(_blockchainName, `Using configured transaction fee of ${_transactionFee} satoshis, because could not determine transaction fee rate: ${err.message}`);
