...
function sendTransaction(account, toAddress, amount, data) {
    ...
    return new Promise((resolve, reject) => {
        ...
        .then(transactionHash => {
            ...
            bcAccounts.updateAccountUtxos(account, utxoInputList);
...
function extractMessage(transaction, blockNumber, timestamp) {
    ...
    return new Promise((resolve, reject) => {
        ...
        for (let vout of transaction.vout) {
            // Check for OP_RETURN
            const data = vout.scriptPubKey.asm;
            if (!data.startsWith(SCRIPTIDENTIFIER)) {
                return reject(new ProcessingError(`No ${SCRIPTIDENTIFIER} with a Whiteflag message found in transaction: ${transaction.hash}`, null, 'WF_API_NO_DATA'));
            }
            // Check for Whiteflag message identifier
            const hexMessage = data.substring(SCRIPTIDENTIFIER.length + 1);
            if (!hexMessage.startsWith(WFINDENTIFIER)) {
                return reject(new ProcessingError(`No Whiteflag message found in ${SCRIPTIDENTIFIER} data of transaction: ${transaction.hash}`, null, 'WF_API_NO_DATA'));
            }
            // Extract Whiteflag message
            try {
                encodedMessage = Buffer.from(hexMessage, BINENCODING).toString(BINENCODING);
            } catch(err) {
                return reject(new Error(`Invalid hexadecimal data in ${SCRIPTIDENTIFIER} of transaction: ${transaction.hash}`));
            }
            // Get additional metadata
            const transactionTime = transaction.time || timestamp;
            ...
            return resolve({
                MetaHeader: {
                    ...
                    transactionTime: new Date(transactionTime * 1000).toISOString(),
 ...
 function signTransaction(account, transaction, bcNetwork) {
    return new Promise((resolve, reject) => {
        ...
        wfState.getKey('blockchainKeys', privateKeyId, function bcGetKeyCb(err, privateKey) {
            ...
            // Get keys in the correct encoding
 ...
 * @private
 * @param {Array} utxoInputList the input UXTOs for the transaction
 * @param {Object} bcNetwork the Bitcoin network parameters
...
function getTotalUtxoInputValue(utxoInputList) {
    return utxoInputList.reduce((accumulator, uxto) => {
 ...
 * @returns {Array} list of unspent UXTOs providing the required amount
 */
