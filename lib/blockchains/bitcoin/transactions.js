...
function sendTransaction(account, toAddress, amount, data = null) {
    ...
    return new Promise((resolve, reject) => {
        ...
        .then(signedTransaction => {
            log.debug(_blockchainName, `Sending signed transaction: ${JSON.stringify(signedTransaction)}`);
 ...
 */
 function calculateTransactionFee(transactionSize) {
    ...
    .then(estimate => {
        return Promise.resolve(Math.ceil((estimate.feerate * SATOSHI) * (transactionSize / KILOBYTE)));
    })
    .catch(err => {
        log.warn(_blockchainName, `Could not determine actual transaction fee rate: ${err.message}`);
        return Promise.resolve(Math.ceil((_bcState.status.feerate * SATOSHI) * (transactionSize / KILOBYTE)));
    });
}
