...
const {UTXOSTATUS} = require('./bitcoinShared');
...
function checkAccountExistsAndSyncstatus(address){
    let account = bitcoinAccountService.getAccount(address);
    if (!account){throw new ProcessingError(`No ${_blockchainName} account found with address: ${address}`);}
    if(account.walletSyncing == true){throw new ProcessingError('wallet is syncing at blocknumber : ' + account.lastBlock + ' , wait until synchronization is finished', null)}
    return account;
}
...
function sendTransaction(account, toAddress, amount, callback) {
    try {
        ...
        bitcoinRpcService.sendSignedTransaction(raw).then(function bitcoinSendSignedTransaction(txHash) {
            if(txHash == null){
                return callback(new ProcessingError('transaction not processed'), null ) }else{
