...
const { retryPromise } = require('../../_common/processing');
...
const STATUSINTERVAL = 120000; // Every two minutes
...
function initRpc(btcConfig, btcState) {
    ...
    // RPC timeout period
    if (Object.hasOwn(btcConfig, 'rpcTimeout') && btcConfig.rpcTimeout > 500) {
    ...
    const rpcCleanURL = getNodeURL(btcConfig, true, DEFAULTPORT);   // no credentials
    _rpcAuthURL = getNodeURL(btcConfig, false, DEFAULTPORT);        // include credentials
    ...
    return new Promise((resolve, reject) => {
        retryPromise(getBlockChainInfo(), CONNECTRETRIES, STATUSINTERVAL)
        .then(btcInfo => {
        ...
        .catch(err => {
            _rpcInit = false;
            reject(new Error(`Could not connect to ${_btcChain} node after ${CONNECTRETRIES} retires: ${err.message}`), _btcChain)
        });
    });
}
