...
function initRpc(btcConfig, btcState) {
    ...
    log.info(_btcName, `Timeout for remote calls to the Bitcoin node: ${_rpcTimeout} ms`);
    ...
    log.trace(_btcName, `Initialising Bitcoin RPC connection with: ${_btcState.parameters.rpcURL}`);
    ...
    return new Promise((resolve, reject) => {
        ...
        .then(btcInfo => {
            ...
            log.info(_btcName, `Connected to Bitcoin ${chain} chain through node: ${_btcState.parameters.rpcURL}`);
        ...
        .catch(err => reject(new Error(`Could not connect to Bitcoin node: ${err.message}`), _btcName));
    });
}
...
async function updateNodeInfo() {
    ...
    .then(btcInfo => {
        if (btcInfo.chain !== _btcChain) {
            log.warn(_btcName, `The node's chain has changed to ${btcInfo.chain} and does not correspond with the configured chain ${_btcChain}`);
        }
        _btcState.parameters.chain = btcInfo.chain;
    })
    .catch(err => log.warn(_btcName, `Could not get network: ${err.message}`));
}
...
async function updateNodeStatus() {
    ...
    .catch(err => log.warn(_btcName, `Could not get peer connection count: ${err.message}`));

    // Get fee rate
    await getFeeRate(1)
    .then(estimate => (_btcState.status.feerate = estimate.feerate))
    .catch(err => log.warn(_btcName, `Could not get transaction fee rate estimate: ${err.message}`));
...
function saveNodeStatus() {
    ...
    log.info(_btcName, `Status: {${JSON.stringify(_btcState.parameters)}, ${JSON.stringify(_btcState.status)}}`);
}
