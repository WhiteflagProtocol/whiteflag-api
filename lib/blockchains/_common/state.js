...
function determineStartingBlock(highestBlock = 0, currentBlock = 0, startingBlock = 0, rewindBlocks = 0) {
    if (startingBlock > 0) return Number(startingBlock - 1);
    if ((highestBlock - rewindBlocks) > (currentBlock + 1)) {
        return Number(highestBlock - rewindBlocks - 1);
    }
 ...
 */
function logStartingBlock(module, blockCursor, highestBlock) {
    let nextBlock = blockCursor + 1;
    let arrearBlocks = highestBlock - nextBlock;
    if (arrearBlocks < 0) {
        log.info(module, `Resuming with next block: ${nextBlock} (when node catches up from its current highest block ${highestBlock})`);
    } else {
        log.info(module, `Resuming with next block: ${nextBlock} (${arrearBlocks} blocks behind the highest known block ${highestBlock})`);
    }
}
