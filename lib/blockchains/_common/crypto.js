 ...
 * @param {*} publicKey the hexadecimal public blockchain key
 * @param {*} publicKeyType the type of public blockchain key
 * @returns 
 */
function verifyJWS(jws, publicKey, publicKeyType) {
    ...
    try {
        ...
        if (err.name === 'JsonWebTokenError'
            ...
            throw new ProtocolError('Invalid JSON Web Signature', [ `${err.name}: ${err.message}` ], 'WF_SIGN_ERROR');
        }
        throw new Error(`Could not JSON Web Signature: ${err.message}`);
    }
}
