...
function verifyJWS(jws, publicKey, publicKeyType) {
    ...
    try {
        ...
        const verificationKey = keyEncoder.encodePublic(publicKey, 'raw', 'pem');
        return jwt.verify(
            jwsCompact,
            verificationKey,
            { allowInvalidAsymmetricKeyTypes: true }
        ...
        if (err.name === 'JsonWebTokenError'
            ...
            throw new ProtocolError(`Invalid JWS: (${err.name}) ${err.message}`, null, 'WF_SIGN_ERROR');
        }
        throw new Error(`Could not verify JWS: ${err.message}`);
    }
}
