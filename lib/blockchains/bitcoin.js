...
async function synchronizeWallet(account) {
    log.info(_blockchainName, 'SYNCHRONIZING WALLET FOR ACCOUNT : ' + account.address + ' CURRENT BLOCK NUMBER ' + account.lastBlock)
...
async function processMultipleBlocks(current, limit) {
    for (current; current <= limit; current++) {
        ...
        log.info(_blockchainName, 'CURRENT BLOCK NUMBER IN BLOCKCHAIN STATE: ' + _currentBlockNumber)
    }
}
...
function createAccount(wif, callback) {
   return bitcoinAccountService.createAccount(wif, callback)
}
