...
function requestKeys(btcPublicKey, callback) {
    log.trace(MODULELOG, `Getting address and encoded keys for Bitcoin public key: ${btcPublicKey}`);
    ...
    try {
        ...
        const publicKeyBuffer = hexToBuffer(btcPublicKey);
        originatorKeys.publicKey.hex = noKeyHexPrefix(btcPublicKey);
        originatorKeys.publicKey.pem = keyEncoder.encodePublic(btcPublicKey, 'raw', 'pem');
...
function requestSignature(wfSignPayload, callback) {
    ...
    .then(account => {
        wfSignPayload.addr = account.address;
 ...
 * @param {wfSignDecoded} wfSignDecoded the full JWS with the Whiteflag signature
 ...
 */
function verifySignature(wfSignDecoded, btcPublicKey, callback) {
    log.trace(MODULELOG, `Validating signature: ${JSON.stringify(wfSignature)}`);
    let result;
    try {
        result = verifyJWS(wfSignDecoded, btcPublicKey, SIGNKEYTYPE);
    } catch(err) {
