...
function initBitcoin(btcConfig, callback) {
    ...
    log.trace(_btcName, 'Initialising the Bitcoin blockchain');

    // Get Bitcoin blockchain state
    wfState.getBlockchainData(_btcName, function blockchainsGetStateDb(err, btcState) {
        if (err) return callback(err, _btcName);
        if (!btcState) {
            log.info(_btcName, 'Creating new Bitcoin entry in internal state');
            btcState = getEmptyState();
            wfState.updateBlockchainData(_btcName, btcState);
        }
        _btcState = btcState;
        ...
        if (btcConfig.testnet) {
            ...
            log.info(_btcName, 'Configured to use the Bitcoin test network');
        } else {
            _btcState.parameters.network = bitcoin.networks.bitcoin;
            log.info(_btcName, 'Configured to use the Bitcoin main network');
        }
        ...
        .then(() => wfState.updateBlockchainData(_btcName, _btcState))
        .then(() => callback(null, _btcName))
        .catch(err => callback(err, _btcName));
    });
}
...
function sendMessage(wfMessage, callback) {
    ...
    .catch(err => {
        log.error(_btcName, `Error sending Whiteflag message: ${err.message}`);
        callback(err);
    });
}
...
function getMessage(wfQuery, callback) {
    ...
    .catch(err => {
        if (err instanceof ProcessingError) {
            log.debug(_btcName, `No Whiteflag message with transaction hash ${transactionHash} found: ${err.message}`);
        } else {
            log.error(_btcName, `Error retrieving Whiteflag message with transaction hash ${transactionHash}: ${err.message}`);
        }
...
function requestSignature(payload, callback) {
    log.trace(_btcName, `Generating signature: ${JSON.stringify(payload)}`);
    ...
    .then(account => {
        ...
        wfState.getKey('blockchainKeys', privateKeyId, function bcGetKeyCb(err, privateKey) {
            ...
            try {
                ...
                log.error(_btcName, `Could not not sign payload: ${err.message}`);
                return callback(err);
            }
...
function requestKeys(publicKey, callback) {
    log.trace(_btcName, `Getting address and encoded keys for public key: ${publicKey}`);
    ...
    try {
        ...
        log.error(_btcName, `Could not get key and address: ${err.message}`);
        return callback(err);
    }
...
function transferFunds(transfer, callback) {
    log.trace(_btcName, `Transferring funds: ${JSON.stringify(transfer)}`);
    ...
    .catch(err => {
        log.error(_btcName, `Error transferring funds: ${err.message}`);
        return callback(err);
    });
}
...
function createAccount(wif, callback) {
    ...
    .then(account => {
        log.info(_btcName, `Bitcoin account created: ${account.address}`);
...
function updateAccount(account, callback) {
    ...
    .then(() => {
        log.info(_btcName, `Bitcoin account updated: ${account.address}`);
        return callback(null, account);
    })
    .catch(err => callback(err));
}
...
function deleteAccount(address, callback) {
    ...
    .then(account => {
        log.info(_btcName, `Bitcoin account deleted: ${account.address}`);
        return callback(null, account);
    })
    .catch(err => callback(err));
}
