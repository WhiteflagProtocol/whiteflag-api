...
function getBlockchainData() {
    return new Promise((resolve, reject) => {
        ...
        bcRpc.getBlockCount().then(async function bitcoinGetBlockNumberCb(latestBlockNumber) {
...
async function synchronizeWallet(account) {
    ...
    for (current; current <= _currentBlockNumber; current++) {
        ...
        await bcRpc.getBlockIncludingTransactions(current).then(async function processTransactionsForAccount(block) {
            if (block != null) {
...
async function getBlockTransactions(current) {
    await bcRpc.getBlockIncludingTransactions(current).then(async function processTransactions(block) {
 ...
 * @private
 * @TODO Function too complex and some variable names used in different scopes
 */
...
function checkTransactionsReceived(transaction, account, index) {
    ...
    if (account.address === transaction.vout[index].scriptPubKey.addresses[0]) {
        if (!account.utxos.some(e => e.txid === transaction.txid)) {
            let utxo = { txid: transaction.txid, index: index, value: parseInt((transaction.vout[index].value * 100000000).toFixed(0)), spent: UTXOSTATUS.UNSPENT }
...
function checkTransactionsSpend(transaction, account, index) {
    for (let utxo of account.utxos) {
        if (utxo.txid == transaction.vin[index].txid && transaction.vin[index].vout == utxo.index && utxo.spent != UTXOSTATUS.SPENTVERIFIED) {
...
function calculateAccountBalance(account) {
    ...
    for (var y=0; y<account.utxos.length; y++) {
        if (account.utxos[y].spent == UTXOSTATUS.UNSPENT) {
...
function transfer(transfer, callback) {
    return bcTransactions.transfer(transfer, callback)
}
