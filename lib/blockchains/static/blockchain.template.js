...
function initBlockchain(bcConfig, callback) {
    ...
    wfState.getBlockchainData(_blockchainName, function blockchainsGetStateDb(err, blockchainState) {
        ...
        if (!blockchainState) {
            log.info(_blockchainName, 'Creating new blockchain entry in internal state');
            blockchainState = getEmptyState();
            wfState.updateBlockchainData(_blockchainName, blockchainState);
        }
