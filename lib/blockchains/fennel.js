...
function initFennel(fnConfig, callback) {
    ...
    wfState.getBlockchainData(_fnlName, function blockchainsGetStateDb(err, fnlState) {
        ...
        fnlRpc.init(fnConfig, _fnlState)
        .then(fnlApi => (_fnlApi = fnlApi))
        .then(() => fnlListener.init(fnConfig, _fnlState))
        .then(() => fnlAccounts.init(fnConfig, _fnlState))
 ...
 */
 function createAccount(seed, callback) {
    ...
    .then(account => {
        log.info(_fnlName, `Fennel account created: ${account.address}`);
...
function updateAccount(account, callback) {
    ...
    .then(updatedAccount => {
        log.info(_fnlName, `Fennel account updated: ${account.address}`);
...
function deleteAccount(address, callback) {
    ...
    .then(account => {
        log.info(_fnlName, `Fennel account deleted: ${account.address}`);
        return callback(null, account);
    })
    .catch(err => callback(err));
}
