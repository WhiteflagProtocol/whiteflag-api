...
function initFennel(fnConfig, callback) {
    ...
    wfState.getBlockchainData(_fnlChain, function blockchainsGetStateDb(err, fnlState) {
        ...
        .then((fnlAPI) => {
            if (fnlAPI) {
                log.info(MODULELOG, `Fennel API initialized with library: ${api.libraryInfo}`);
                _fnlApi = fnlAPI;
            }
            return Promise.resolve()
        })
        .then(() => fnlListener.init(fnConfig, _fnlState, _fnlApi))
...
function getMessage(wfQuery, callback) {
    ...
    let wfMessage = null;
    return callback(new ProcessingError('Function to query messages on the blockchain not yet implemented', null, 'WF_API_NOT_IMPLEMENTED'), wfMessage);
}
...
function getBinaryAddress(fnlAddress, callback) {
    let addressBuffer;
    try {
        addressBuffer = dotCrypto.decodeAddress(fnlAddress);
    } catch(err) {
