...
const { bufferToBase64u, hexToBase64u } = require('../common/encoding');
 ...
 * @param {Object} payload the JWS payload for the Whiteflag signature
...
function requestSignature(payload, callback) {
    const input = jws.createSignInput(payload, SIGNALGORITHM)
    fnlAccounts.sign(payload.addr, input)
    .then(output => {
