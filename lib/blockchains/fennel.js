...
function initFennel(fnConfig, callback) {
    ...
    wfState.getBlockchainData(_fnlChain, function blockchainsGetStateDb(err, fnlState) {
        ...
        .then((accountScheduleId) => {
            _accountScheduleId = accountScheduleId;
            return fnlRpc.init(fnConfig, _fnlState)
        })
        .then((fnlAPI) => {
        ...
        .catch((err) => {
            clearInterval(_accountScheduleId);
            return callback(err, _fnlChain);
        });
    });
}
