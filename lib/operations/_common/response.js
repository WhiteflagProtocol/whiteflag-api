 ...
 * @param {string} [resourceType] the type of resource returned
 * @param {string} [resourceId] the identifier of the resource, if available
 * @returns {Object} the response body
 */
function createBody(req, operationId = null, resourceType = null, resourceId = null) {
    let resBody = {};
    resBody.meta = {
        ...
        request: {
            client: req.ip,
            method: req.method,
            url: getURL(req) + req.originalUrl
        }
    }
    if (resourceType) {
        ...
        if (resourceId) {
            resBody.meta.resource.id = resourceId;
        }
    }
    return resBody;
}
 ...
 * @param {Object} [meta] the meta data object to add message details to
 * @returns {Object} the updated meta data object
 */
function addMessageData(wfMessage, meta = {}) {
    if (!meta.resource) {
        meta.resource = { type: 'message' };
    }
    meta.resource.code = msg.type(wfMessage);
    meta.resource.class = msg.title(wfMessage);
...
function sendIndicative(res, err, resBody, resData, callback) {
    ...
    resBody.meta = addItemCount(resData, resBody.meta);
