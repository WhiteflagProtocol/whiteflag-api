...
function decodeSignature(req, res, operationId, callback) {
     ...
     */
    wfAuthenticate.decodeSignature(wfSignature, function endpointSignatureDecodeCb(err, wfSignDecoded) {
        ...
        return response.sendIndicative(res, err, resBody, wfSignDecoded, callback);
    });
}
...
function verifySignature(req, res, operationId, callback) {
     ...
     */
    wfAuthenticate.verifySignature(wfExtSignature, function endpointSignatureVerifyCb(err, wfSignDecoded) {
        // Create response body and preserve information before responding
        let resBody = response.createBody(req, operationId);
        if (!err) {
