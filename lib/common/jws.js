...
const { isObject, isString, base64uToString, stringToBase64u } = require('./encoding');
...
function createSignInput(payload, algorithm) {
    ...
    return (
        stringToBase64u(JSON.stringify(jws.protected))
        + JWSSEPARATOR
        + stringToBase64u(JSON.stringify(jws.payload))
    );
}
