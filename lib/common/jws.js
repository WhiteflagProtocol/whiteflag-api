...
function createSignInput(payload, algorithm) {
    ...
    return (
        Buffer.from(JSON.stringify(jws.protected)).toString('base64url')
        + JWSSEPARATOR
        + Buffer.from(JSON.stringify(jws.payload)).toString('base64url')
    );
}
...
function toBase64u(object) {
    return Buffer.from(JSON.stringify(object), 'utf-8').toString('base64url');
}
 ...
 * @param {string} string a base64URL encoded JSON string
