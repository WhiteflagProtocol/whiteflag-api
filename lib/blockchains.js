...
function sendMessage(wfMessage, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        ...
        if (blockchainInstance.name === wfMessage.MetaHeader.blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.sendMessage(wfMessage, blockchainSendMessageCb);
            } else {
                return callback(new ProcessingError(`Blockchain not active: ${blockchainInstance.name}`, null, 'WF_API_NOT_AVAILABLE'), wfMessage);
            }
        }
    });
    /**
     ...
     * @param {Error} err error object if any error
     * @param {string} transactionHash the hexadecial representation of the transaction hash of the sent message
     * @param {number} blockNumber the block number of the sent message
     */
...
function getMessage(wfQuery = {}, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        ...
        if (blockchainInstance.name === wfQuery['MetaHeader.blockchain']) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.getMessage(wfQuery, blockchainLookupMessageCb);
            } else {
                return callback(new ProcessingError(`Blockchain not active: ${blockchainInstance.name}`, null, 'WF_API_NOT_AVAILABLE'));
            }
        }
    });
    /**
...
function requestSignature(signPayload = {}, blockchain, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        ...
        if (blockchainInstance.name === blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.requestSignature(signPayload, blockchainRequestSignatureCb);
            } else {
                return callback(new ProcessingError(`Blockchain not active: ${blockchainInstance.name}`, null, 'WF_API_NOT_AVAILABLE'));
            }
        }
...
function requestKeys(originatorPubKey, blockchain, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        ...
        if (blockchainInstance.name === blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.requestKeys(originatorPubKey, blockchainRequestKeysCb);
            } else {
                return callback(new ProcessingError(`Blockchain not active: ${blockchainInstance.name}`, null, 'WF_API_NOT_AVAILABLE'));
            }
        }
     ...
     * @param {Error} err error object if any error
     * @param {originatorKeys} originatorKeys the originator address and keys
     */
...
function getBinaryAddress(address, blockchain, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        ...
        if (blockchainInstance.name === blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.getBinaryAddress(address, blockchainBinaryAddressCb);
            } else {
                return callback(new ProcessingError(`Blockchain not active: ${blockchainInstance.name}`, null, 'WF_API_NOT_AVAILABLE'));
            }
        }
...
function transferFunds(transfer = {}, address, blockchain, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        if (blockchainInstance.name === blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.transfer(transfer, blockchainTransferValueCb);
            } else {
                return callback(new ProcessingError(`Blockchain not active: ${blockchainInstance.name}`, null, 'WF_API_NOT_AVAILABLE'));
            }
        }
...
function createAccount(blockchain, privateKey = null, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        if (blockchainInstance.name === blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.createAccount(privateKey, function blockchainCreateAccountCb(err, result) {
                    if (err) return callback(err);
...
function updateAccount(account = {}, address, blockchain, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        if (blockchainInstance.name === blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.updateAccount(account, function blockchainUpdateAccountCb(err, result) {
                    if (err) return callback(err);
                    return callback(null, result);
                });
            } else {
...
function deleteAccount(address, blockchain, callback) {
    ...
    _blockchainsConfig.forEach(blockchainInstance => {
        if (blockchainInstance.name === blockchain) {
            if (blockchainInstance._enabled) {
                blockchainInstance._moduleImpl.deleteAccount(address, function blockchainDeleteAccountCb(err, result) {
                    if (err) return callback(err);
                    return callback(null, result);
                });
            } else {
...
function enableBlockchain(blockchainInstance) {
    ...
    if (!blockchainInstance.active) return false;
