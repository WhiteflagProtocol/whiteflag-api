 ...
 * @param {blockhainsInitCb} callback function to be called upon completion
 */
function initBlockchains(callback) {
    ...
    _bcConfig.forEach(bcInstance => {
        if (bcInstance._enabled) {
             ...
             */
            bcInstance._moduleImpl.init(bcInstance, function bcInitCb(err, blockchain) {
...
function sendMessage(wfMessage, callback) {
    ...
    _bcConfig.forEach(bcInstance => {
        ...
        if (bcInstance.name === wfMessage.MetaHeader.blockchain) {
             ...
             */
            bcInstance._moduleImpl.sendMessage(wfMessage, function bcSendMessageCb(err, transactionHash, blockNumber) {
                if (err) {
                    wfMessage.MetaHeader.transmissionSuccess = false;
 ...
 * @param {bcGetMessageCb} callback function to be called upon completion
 */
 ...
 */
function requestKeys(orgPubkey, blockchain, callback) {
    // Check parameters
    ...
    _bcConfig.forEach(bcInstance => {
        ...
        if (bcInstance.name === blockchain) {
            ...
            bcInstance._moduleImpl.requestKeys(orgPubkey, callback);
        }
    });
}
 ...
 * @function requestSignature
 * @alias module:lib/blockchains.requestSignature
 * @param {Object} wfSignature the JWS payload for the Whiteflag signature
 * @param {string} blockchain the blockchain for which the signature is requested
 * @param {authVerifySignatureCb} callback function to be called upon completion
 */
function verifySignature(wfSignature, orgPubkey, blockchain, callback) {
    // Check blockchain
    if (!blockchain || !_blockchains.includes(blockchain)) {
        return reportUnsupported(blockchain, callback);
    }
    ...
    let payloadErrors = [];
    if (!wfSignDecoded.payload) payloadErrors.push('No payload in signature');
    if (!wfSignDecoded.payload.addr) payloadErrors.push('No blockchain address specified in payload');
    if (!wfSignDecoded.payload.orgname) payloadErrors.push('No originator name specified in payload');
    if (!wfSignDecoded.payload.url) payloadErrors.push('No URL specified in payload');
    ...
    _bcConfig.forEach(bcInstance => {
        ...
        if (bcInstance.name === blockchain) {
            if (!bcInstance._enabled) return reportNotActive(bcInstance.name, callback);
            bcInstance._moduleImpl.verifySignature(wfSignDecoded, orgPubkey, callback);
        }
    });
}
