...
const BCCONFFILE = './config/blockchains.toml';
...
function initBlockchains(callback) {
    ...
    function blockchainInitCb(err, blockchain) {
        if (err) {
            log.fatal(MODULELOG, `Cannot intialise ${blockchain}: ${err.message}`);
...
function sendMessage(wfMessage, callback) {
    ...
    if (!wfMessage.hasOwnProperty('MetaHeader')) {
...
function confirmMessage(wfMessage) {
    ...
    wfState.updateQueueData('blockDepths', 'transactionHash', message);
    ...
    getBlockHeight(message.blockchain, function blockchainGetHeightCb(err, blockHeight) {
        ...
        bcConfig._moduleImpl.getTransaction(message.transactionHash, function blockchainTransactionConfirmCb(err, wfMessage) {
            ...
            if (message.blockNumber !== wfMessage.MetaHeader.blockNumber) {
                ...
                return wfState.updateQueueData('blockDepths', 'transactionHash', message);
            }
...
function updateMessageConfirmation(message) {
    ...
    wfApiDatastores.getMessages(wfQuery, function blockchainGetMessageDbCb(err, wfMessages, count) {
        ...
        if (wfMessage.MetaHeader.transactionHash === message.transactionHash) {
            ...
            if (message.confirmed) wfMessage.MetaHeader.confirmed = message.confirmed;

            // Store updated message is datastores
            wfApiDatastores.storeMessage(wfMessage, function blockchainConfirmStoreMessageCb(err, result) {
                if (err) return log.error(err);
                ignore(result);

                // Remove from queue if message has been confirmed and stored
                if (message.confirmed) return removeMessageConfirmation(message);
            });
        }
    });
}
