...
const BCSCHEMADIR = './lib/blockchains/_static/'
const BCSCHEMAFILE = BCSCHEMADIR + 'blockchains.config.schema.json'
...
function initBlockchains(callback) {
    ...
    try {
        ...
        return callback(new Error(`Could not read configuration from ${BCCONFFILE}: ${err.message}`));
    }
    try {
        _bcConfigSchema = JSON.parse(fs.readFileSync(BCSCHEMAFILE));
    } catch(err) {
        return callback(new Error(`Could not read configuration schema from ${BCSCHEMAFILE}: ${err.message}`));
    }
    // Parse the configuration
    if (!parseConfig(bcConfigData)) {
        return callback(new Error(`Could not parse configuration from ${BCCONFFILE}`));
    }
