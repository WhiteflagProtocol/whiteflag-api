 ...
 * @param {*} blockchain the blockchainto scan
...
function scanBlocks(firstBlock, lastBlock, blockchain, callback) {
    ...
    _bcConfig.forEach(bcInstance => {
        ...
        if (bcInstance.name === blockchain) {
             ...
             * @param {Error} err any error
             * @param {WfMessage[]} messages an array of found Whiteflag messages
             */
    ...
    _bcConfig.forEach(bcInstance => {
        ...
        if (bcInstance.name === wfMessage.MetaHeader.blockchain) {
            ...
            bcInstance._module.sendMessage(wfMessage, function bcSendMessageCb(err, transactionHash, blockNumber) {
                ...
                if (transactionHash) wfMessage.MetaHeader.transactionHash = transactionHash;
                if (blockNumber) wfMessage.MetaHeader.blockNumber = blockNumber;
...
function initConfirmMessages(confirmationConfig = {}) {
    ...
    if (confirmationConfig.doubleCheck) _confirmDoubleCheck = confirmationConfig.doubleCheck;
...
function checkMessageConfirmation(message) {
    getBlockHeight(message.blockchain, function blockchainGetHeightCb(err, blockHeight) {
        ...
        if (!_confirmDoubleCheck) {
            ...
            return updateMessageConfirmation(message);
        }
        ...
        bcInstance._module.getMessage(wfQuery, function blockchainTransactionConfirmCb(err, messageDuplicate) {
            if (err) return log.error(MODULELOG, err.message);
