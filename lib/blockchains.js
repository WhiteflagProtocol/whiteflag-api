...
function getMessage(wfQuery = {}, callback) {
    ...
    _blockchainsConfig.forEach(bcConfig => {
        ...
        if (bcConfig.name === wfQuery['MetaHeader.blockchain']) {
            if (bcConfig._enabled) {
                bcConfig._moduleImpl.getMessage(wfQuery, blockchainLookupMessageCb);
            } else {
...
function enableBlockchain(bcConfig) {
    ...
    try {
        ...
        log.error(MODULELOG, `Module ${bcConfig.module} cannot be loaded: ${err.message}`);
        return false;
    }
    // Blockchain enabled
    return true;
...
function checkMessageConfirmation(message) {
    getBlockHeight(message.blockchain, function blockchainGetHeightCb(err, blockHeight) {
        ...
        bcConfig._moduleImpl.getMessage(message.transactionHash, function blockchainTransactionConfirmCb(err, wfMessage) {
            if (err) return log.error(MODULELOG, err.message);
