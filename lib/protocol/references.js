'use strict';
/**
 * @module lib/protocol/validation
 * @summary Whiteflag message validation module
 * @description Module for message format, reference and originator verification functions
 */
module.exports = {
...
function evaluateReference(wfMessage, wfRefMessage, callback) {
    log.trace(MODULELOG, `Evaluating reference from message ${wfMessage.MetaHeader.transactionHash}`);
    ...
    try {
        ...
        if (mesageTypeIndex < 0) {
            referenceErrors.push(`Message type ${header.MessageCode} is not allowed`);
        } else {
    ...
    if (referenceErrors.length > 0) {
        wfMessage.MetaHeader.messageErrors = array.addArray(wfMessage.MetaHeader.messageErrors, referenceErrors);
        return callback(new ProtocolError('Invalid message reference', referenceErrors, 'WF_REFERENCE_ERROR'));
    }
    return callback(null);
}
