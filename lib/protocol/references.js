...
function evaluateReference(wfMessage, wfRefMessage, callback) {
    ...
    try {
        ...
        if (allowedReferenceIndex < 0) {
            referenceErrors.push(`Message type ${header.MessageCode} may not reference message type ${refHeader.MessageCode}`);
        } else {
            ...
            if (!wfSpecAllowedReferences.includes(header.ReferenceIndicator)) {
                referenceErrors.push(`Message type ${header.MessageCode} may not use reference code ${header.ReferenceIndicator} to reference message type ${refHeader.MessageCode}`);
            }
            ...
            if (refIndicatorIndex < 0) {
                ...
                if (!wfSpecAllowedRefIndicators.includes(refHeader.ReferenceIndicator)) {
                    referenceErrors.push(`Reference code ${header.ReferenceIndicator} cannot meaningfully reference reference code ${refHeader.ReferenceIndicator}`);
                }
            }
        }
    } catch(err) {
    ...
    if (referenceErrors.length > 0) {
        wfMessage.MetaHeader.validationErrors = arr.addArray(wfMessage.MetaHeader.validationErrors, referenceErrors);
