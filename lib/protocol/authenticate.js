...
/**
 * Requests a authentication token for the specified blockchain address
...
function generateToken(authToken, address, blockchain, callback) {
    ...
    wfApiBlockchain.getBinaryAddress(address, blockchain, function authGetAddressCb(err, addressBuffer) {
        if (err) return callback(err, null);

        // Generate authentication token and compare with message
        wfCrypto.generateAuthToken(tokenBuffer, addressBuffer, function authGenerateTokenCb(err, tokenBuffer) {
            if (err) return callback(err, null);
            return callback(null, tokenBuffer.toString('hex').toLowerCase());
        });
    });
}
    ...
    getSignature(authURL, function authGetSignatureCb(err, wfSignature) {
        ...
        verifySignature(wfExtSignature, function authVerifySignatureCb(err, wfSignatureDecoded, originatorKeys) {
            ...
            if (authenticationErrors.length > 0) {
                ...
                let err = new ProtocolError(`Originator of ${type(wfAuthMessage)} message ${wfAuthMessage.MetaHeader.transactionHash} could not be authenticated`, authenticationErrors, 'WF_AUTH_ERROR');
                return callback(err, wfAuthMessage);
            }
