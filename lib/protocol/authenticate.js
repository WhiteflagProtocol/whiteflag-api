...
function wfAuthentication1(wfAuthMessage, callback) {
    ...
    retrieveSignature(authURL, function authGetSignatureCb(err, wfSignature) {
        ...
        verifySignature(wfExtSignature, function authVerifySignatureCb(err, wfSignDecoded) {
            ...
            if (!wfSignDecoded) {
                log.debug(MODULELOG, `Could not decode and verify signature from ${body.VerificationData}`);
                meta.validationErrors = arr.addItem(
                    meta.validationErrors,
            ...
            log.trace(MODULELOG, `Verified signature from ${body.VerificationData}: ` + JSON.stringify(wfSignDecoded));
            ...
            if (wfSignDecoded.addr.toLowerCase() !== meta.originatorAddress.toLowerCase()) {
                authErrors.push(`Signature address does not correspond with message address: ${wfSignDecoded.addr} != ${meta.originatorAddress}`);
                meta.originatorValid = false;
            }
            if (wfSignDecoded.url !== body.VerificationData) {
                authErrors.push(`Signature URL does not correspond with authentication message URL: ${wfSignDecoded.url} != ${body.VerificationData}`);
                meta.originatorValid = false;
            }
            ...
            if (authErrors.length > 0) {
                ...
                meta.originatorValid = false;
                meta.validationErrors = arr.addArray(meta.validationErrors, authErrors);
