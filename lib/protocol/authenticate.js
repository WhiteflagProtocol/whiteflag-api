...
function decodeSignature(wfSignature, callback) {
    ...
    let signErrors = [];
    if (!wfSignature?.protected) signErrors.push('Missing Whiteflag JWS property: protected');
    if (!wfSignature.payload) signErrors.push('Missing Whiteflag JWS property: payload');
    if (!wfSignature.signature) signErrors.push('Missing Whiteflag JWS property: signature');
    if (signErrors.length > 0) {
        return callback(new ProtocolError('Invalid Whiteflag authentication signature', signErrors, 'WF_SIGN_ERROR'), null);
    }
    let wfSignDecoded;
    try {
        wfSignDecoded = jws.toFull(wfSignature);
    } catch(err) {
        return callback(new ProtocolError('Invalid Whiteflag authentication signature', err.message, 'WF_SIGN_ERROR'), null)
    }
...
function verifySignature(wfExtSignature, callback) {
    ...
    if (signErrors.length > 0) {
        return callback(new ProcessingError('Invalid extended Whiteflag authentication signature data', signErrors), null);
    }
    // Check properties of the signature object
     ...
     */
    wfApiBlockchains.verifySignature(wfSignature, orgPubkey, blockchain, function authVerifySignatureCb(err, wfSignDecoded) {
