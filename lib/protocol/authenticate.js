...
const { type } = require('../_common/protocol');
 ...
 * @param {function(Error, wfSignature, wfSignDecoded)} callback function to be called upon completion
 */
function createSignature(signPayload, address, blockchain, callback) {
    ...
    array.addArray(signErrors, checkSignPayload(signPayload));
 ...
 * @param {function(Error, wfSignDecoded)} callback function to be called upon completion
 * @todo Detailed error processing after JWT decoding
 */
function decodeSignature(signature, callback) {
    // Determine if Whiteflag signature or extended signature
    let wfSignature;
    if (Object.hasOwn(signature, 'wfSignature')) {
        wfSignature = signature.wfSignature;
    } else {
        wfSignature = signature;
    }
    // Decode the Whiteflag signature properties
    try {
 ...
 * @param {function(Error, wfSignDecoded, Object)} callback function to be called upon completion
 */
function verifySignature(wfExtSignature, callback) {
    // Check properties of the extended signature object
    let signErrors = [];
    if (!wfExtSignature?.blockchain) signErrors.push('Missing property: blockchain');
    if (!wfExtSignature?.originatorPubKey) signErrors.push('Missing property: originatorPubKey');
    if (!wfExtSignature?.wfSignature) signErrors.push('Missing property: wfSignature');
    if (signErrors.length > 0) {
    ...
    const wfSignature = wfExtSignature.wfSignature;
    decodeSignature(wfSignature, function(err, wfSignDecoded){
        ...
        const orgPubkey = wfExtSignature.originatorPubKey;
        /**
         * @callback authVerifySignatureCb
         * @param {Error} err any error
         * @param {wfSignDecoded} wfSignDecoded the verified and decoded signature
         */
        wfApiBlockchains.verifySignature(wfSignature, signAddress, orgPubkey, blockchain, function authVerifySignatureCb(err, result) {
            if (err) return callback(err);
            if (result) return callback(null, wfSignDecoded, orgPubkey);
            return callback(null, wfSignDecoded, orgPubkey);
        });
    });
}
...
function wfAuthentication1(wfAuthMessage, callback) {
    ...
    retrieveSignature(authURL, function authGetSignatureCb(err, wfSignature) {
        ...
        verifySignature(wfExtSignature, function authVerifySignatureCb(err, wfSignDecoded, originatorKeys) {
            if (err) {
            ...
            if (wfSignDecoded.addr.toLowerCase() !== originatorKeys.address.toLowerCase()) {
                authenticationErrors.push(`Signature address does not correspond with derived blockchain address: ${wfSignDecoded.addr} != ${originatorKeys.address}`);
                wfAuthMessage.MetaHeader.originatorValid = false;
            }
