...
function verifyFormat(wfMessage, callback) {
    ...
    delete meta.formatValid;
    
    // Check for message components
    let formatErrors = [];
    if (!wfMessage.MessageHeader) formatErrors.push('Missing message header');
    if (!wfMessage.MessageBody) formatErrors.push('Missing message body');
    ...
    try {
        ...
        formatErrors = formatErrors.concat(
            arr.pluck(jsonValidate(wfMessage, wfMsgSchema).errors, 'stack')
        );
        // Check field vaues
        formatErrors = arr.addArray(formatErrors, verifyValues(wfMessage));
    } catch(err) {
 ...
 * @param {decodeMessageCb} callback function called on completion
 */
function decodeMessage(wfMessage, callback) {
    ...
    if (!meta.encodedMessage) {
        ...
        meta.encodedMessage = noHexPrefix(meta.encodedMessage);
    }
    // Create character string of the binary representation
    const encryptedMessage = hexToBuffer(meta.encodedMessage);
    ...
    switch (header.EncryptionIndicator) {
        ...
        case '2': {
            ...
            if (!meta.encryptionInitVector) {
                 ...
                 * @param {wfMessage} message the Whiteflag message
                 * @param {boolean} ivMissing true if iv is missing else false
                 */
...
function verifyValues(wfMessage) {
    ...
    const { MessageHeader: header, MessageBody: body } = wfMessage;
    const wfSpec = wfMsgSchema.specifications;
    let contentErrors = [];
...
function encodeBDX2BinStr(fieldStr, nBits) {
    ...
    for (const bdx of fieldStr) {
        const binCNum = parseInt(bdx, 16).toString(2);
...
function encodeDatum2BinStr(fieldStr) {
    ...
    for (const char of fieldStrCl) {
        const binCNum = parseInt(char, 10).toString(2);
        binStr += (padbits + binCNum).slice(-padding);
    }
    return (sign + binStr);
}
 ...
 * @private
 * @param {Buffer} buffer binary encoding of a message
...
function decodeBin2BinStr(buffer) {
    ...
    for (const byte of buffer) {
        const binChars = byte.toString(2);
