...
function verifyFormat(wfMessage, callback) {
    ...
    delete wfMessage.MetaHeader.formatValid;
    ...
    try {
        ...
        formatErrors = formatErrors.concat(
            arr.pluck(jsonValidate(wfMessage, wfMessageSchema).errors, 'stack')
        );
        // Check field vaues
...
function decodeMessage(wfMessage, callback) {
    ...
    if (!wfMessage.MetaHeader.encodedMessage) {
        ...
        wfMessage.MetaHeader.encodedMessage = noHexPrefix(wfMessage.MetaHeader.encodedMessage);
    }
    // Create character string of the binary representation
    const encryptedMessage = hexToBuffer(wfMessage.MetaHeader.encodedMessage);
