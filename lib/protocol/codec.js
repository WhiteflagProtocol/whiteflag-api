...
function encodeMessage(wfMessage, callback) {
    ...
    verifyFormat(wfMessage, function encodeVerifyFormatCb(err, wfMessage) {
        ...
        switch (header.MessageCode) {
            ...
            case 'R': {
                bodyBinStr = (
                    encodeBDX2BinStr(body.ResourceMethod, 4) +
                    encodeUTF2BinStr(body.ResourceData, 0)
                );
                break;
            }
            case 'F': {
...
function decodeMessage(wfMessage, callback) {
    ...
    wfCrypto.decrypt(wfMessage, encryptedMessage, function decodeDecryptCb(err, decryptedMessage) {
        ...
        switch (header.MessageCode) {
            ...
            case 'R': {
                body.ResourceMethod = decodeBinStr(messageBinStr, 301, 305, 'hex').toUpperCase();
                body.ResourceData = decodeBinStr(messageBinStr, 305, messageLength, 'utf');
                break;
            }
            case 'F': {
