 ...
 * @todo encoding and decoding of test messages
 */
module.exports = {
    // Codec functions
 ...
 * @todo specific validation of message body against sub-schema definitions
 */
 ...
 * @param {object} wfMessage a Whiteflag message
 * @param {function} callback function to be called upon completion
 * @todo encoding of test messages
 */
function encodeMessage(wfMessage, callback) {
    ...
    verifyFormat(wfMessage, function encodeVerifyFormatCb(err, wfMessage) {
        ...
        switch (header.MessageCode) {
            case 'A': {
                bodyBinStr = (
                    encodeBDX2BinStr(body.VerificationMethod, 4) +
                    encodeUTF2BinStr(body.VerificationData, 0)
                );
                break;
            }
            case 'K': {
                bodyBinStr = (
                    encodeBDX2BinStr(body.CryptoDataType, 8) +
                    encodeBDX2BinStr(body.CryptoData, (body.CryptoData.length * 4))
                );
                break;
            }
            case 'Q': {
                if (body.requestObjects) {
                    body.requestObjects.forEach(requestObject => {
            ...
            case 'M': {
                bodyBinStr = (
                    encodeBDX2BinStr(body.SubjectCode, 8) +
                    encodeDatum2BinStr(body.DateTime) +
                    encodeDatum2BinStr(body.Duration) +
                    encodeBDX2BinStr(body.ObjectType, 8) +
                    encodeDatum2BinStr(body.ObjectLatitude) +
                    encodeDatum2BinStr(body.ObjectLongitude) +
                    encodeBDX2BinStr(body.ObjectSizeDim1, 16) +
                    encodeBDX2BinStr(body.ObjectSizeDim2, 16) +
                    encodeBDX2BinStr(body.ObjectOrientation, 12) +
                    requestObjectsBinStr
                );
                break;
            }
            case 'R': {
                bodyBinStr = (
                    encodeBDX2BinStr(body.ReferenceMethod, 4) +
                    encodeUTF2BinStr(body.ReferenceData, 0)
                );
                break;
            }
            case 'F': {
                bodyBinStr = encodeUTF2BinStr(body.Text, 0);
                break;
            }
            case 'T': {
                // TODO: implement messages types
                return callback(new ProcessingError(`Encoding of message type ${header.MessageCode} not implemented`, null, 'WF_API_NOT_IMPLEMENTED'), wfMessage);
            }
            default: {
                return callback(new ProtocolError(`Invalid message type: ${header.MessageCode}`), wfMessage);
            }
        }
 ...
 * @param {object} wfMessage Whiteflag message with encoded message string in MetaHeader
...
function decodeMessage(wfMessage, callback) {
    ...
    wfCrypto.decrypt(wfMessage, encryptedMessage, function decodeDecryptCb(err, decryptedMessage) {
        ...
        switch (header.MessageCode) {
            case 'A': {
                body.VerificationMethod = decodeBinStr(messageBinStr, 301, 305, 'hex').toUpperCase();
                body.VerificationData = decodeBinStr(messageBinStr, 305, messageLength, 'utf');
                break;
            }
            case 'K': {
                body.CryptoDataType = decodeBinStr(messageBinStr, 301, 309, 'hex').toUpperCase();
                body.CryptoData = decodeBinStr(messageBinStr, 309, (messageLength + 4), 'hex');
                break;
            }
            case 'P':
            case 'D':
            ...
            case 'M':
            case 'Q': {
                body.SubjectCode = decodeBinStr(messageBinStr, 301, 309, 'hex').toUpperCase();
                body.DateTime = decodeBinStr(messageBinStr, 309, 365, 'datetime');
                body.Duration = decodeBinStr(messageBinStr, 365, 389, 'duration');
                body.ObjectType = decodeBinStr(messageBinStr, 389, 397, 'hex').toUpperCase();
                body.ObjectLatitude = decodeBinStr(messageBinStr, 397, 426, 'lat');
                body.ObjectLongitude = decodeBinStr(messageBinStr, 426, 459, 'long');
                body.ObjectSizeDim1 = decodeBinStr(messageBinStr, 459, 475, 'dec');
                body.ObjectSizeDim2 = decodeBinStr(messageBinStr, 475, 491, 'dec');
                body.ObjectOrientation = decodeBinStr(messageBinStr, 491, 503, 'dec');
                if (header.MessageCode === 'Q') {
                    body.requestObjects = [];
                    for (let b = 503; b <= messageLength; b += (2 * BYTELENGTH)) {
                        let requestObject = {};
                        ...
                        body.requestObjects.push(requestObject);
                    }
                }
                break;
            }
            case 'R': {
                body.ReferenceMethod = decodeBinStr(messageBinStr, 301, 305, 'hex').toUpperCase();
                body.ReferenceData = decodeBinStr(messageBinStr, 305, messageLength, 'utf');
                break;
            }
            case 'F': {
                body.Text = decodeBinStr(messageBinStr, 301, messageLength, 'utf');
                break;
            }
            case 'T': {
                // TODO: implement messages types
                return callback(new ProcessingError(`Encoding of message type ${header.MessageCode} not implemented`, null, 'WF_API_NOT_IMPLEMENTED'), wfMessage, false);
            }
            default: {
