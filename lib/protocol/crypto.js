...
/**
 * Generates authentication token from shared secret authentication token
 * @function generateAuthToken
 * @alias module:lib/protocol/crypto.generateAuthToken
 * @param {buffer} token binary encoded secret authentication token as input key material
 * @param {buffer} address binary encoded blockchain address by which the token is used
 * @param {function} callback function to be called upon completion
 */
function generateAuthToken(authToken, address, callback) {
    // Check input buffers
    ...
    const salt = Buffer.from(authenticationParameters.salt[AUTHMETHOD], BINENCODING);
    const token = hkdf(authToken, salt, address, tokenlength);
    authToken = undefined;
