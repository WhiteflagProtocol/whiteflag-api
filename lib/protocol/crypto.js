...
function encryptMessage(wfMessage, encodedMessage, callback) {
    ...
    switch (method) {
        ...
        case '1': {
            ...
            const metaheaderErrors = checkMetaHeaderErrors(wfMessage);
...
/**
 * Check message meta header for errors
 * @private
 * @param {wfMessage} wfMessage
 * @returns {Array} message metaheader errors
 */
function checkMetaHeaderErrors(wfMessage) {
    let metaheaderErrors = [];
    if (!wfMessage?.MetaHeader?.blockchain) metaheaderErrors.push('Blockchain not specified in metaheader');
    if (!wfMessage?.MetaHeader?.originatorAddress) metaheaderErrors.push('Originator address not specified in metaheader');
    if (!wfMessage?.MetaHeader?.recipientAddress) metaheaderErrors.push('Recipient address not specified in metaheader');
    return metaheaderErrors;
}
