...
function getAuthMessages(originatorAddress = '0', blockchain = null, callback) {
    ...
    retrieveMessages(wfQuery, blockchain, function retrieveAuthMessagesCb(err, authMessageSequence) {
        ...
        if (err && authMessageSequence.length === 0) {
        ...
        // TODO: authMessageSequence = removeSemanticReferences(authMessageSequence);
        // TODO: return callback(null, removeSupersededMessages(authMessageSequence));
            ...
            function iterateMessages(i) {
                ...
                retrieveMessages(wfQuery, blockchain, function retrieveSequenceRefMessagesCb(err, refMessages) {
                    ...
                    if (wfMessageSequence.length > sequenceBeginLength) {
                        // TODO: return iterateSeqence(removeSemanticReferences(wfMessageSequence));
                        return iterateSeqence(wfMessageSequence);
                    }
                    // No more referencing messages; clean up sequence and return result
                    log.trace(MODULELOG, `Found sequence of ${wfMessageSequence.length} messages: removing superseded messages`);
                    // TODO: return callback(null, removeSupersededMessages(wfMessageSequence));
...
function retrieveMessages(wfQuery = {}, blockchain = null, callback) {
    ...
    wfApiDatastores.getMessages(wfQuery, function retrieveMessagesDbCb(err, wfMessages, count) {
        ...
        if (blockchain) wfQuery['MetaHeader.blockchain'] = blockchain;
        wfApiBlockchain.lookupMessage(wfQuery, function retrieveMessagesBcCb(err, wfMessage) {
 ...
 * @todo This function needs to be evaluated and tested
 * @private
