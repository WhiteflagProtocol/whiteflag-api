...
function getMessage(transactionHash = '0', blockchain = null, callback) {
    ...
    // Construct query to retrieve single message
    wfQuery['MetaHeader.transactionHash'] = transactionHash;
    if (blockchain) wfQuery['MetaHeader.blockchain'] = blockchain;

    // Call function to retrieve single message and pass it to callback
    return retrieveSingleMessage(wfQuery, blockchain, callback);
}
...
function getQuery(wfQuery = {}, callback) {
    // Call function to retrieve single message and pass it to callback
    return retrieveMultipleMessages(wfQuery, null, callback);
}
...
function getReferences(transactionHash = '0', blockchain = null, callback) {
    ...
    // Construct query to retrieve references
    wfQuery['MessageHeader.ReferencedMessage'] = transactionHash;
    if (blockchain) wfQuery['MetaHeader.blockchain'] = blockchain;

    // Call function to retrieve referencing messages and pass them to callback
    return retrieveMultipleMessages(wfQuery, blockchain, callback);
}
...
function getAuthMessages(originatorAddress = '0', blockchain = null, callback) {
    ...
    retrieveMultipleMessages(wfQuery, blockchain, function retrieveAuthMessagesCb(err, authMessageSequence) {
        // Check for errors and if any authentication message was found
        if (err && !authMessageSequence) {
            if (err) return callback(err);
            return callback(new Error('Could not find transaction hash of first message in sequence'));
        }
        // Return result, removing semantic refs and superseded messages from sequence
        authMessageSequence = removeSemanticReferences(authMessageSequence);
        return callback(null, removeSupersededMessages(authMessageSequence));
    });
}
...
function getSequence(transactionHash = '0', blockchain = null, callback) {
    ...
    retrieveSingleMessage(wfQuery, blockchain, function retrieveSequenceCb(err, wfMessageInitSequence) {
        // Check if transaction hash of message was found
        if (err) return callback(err);
        if (!wfMessageInitSequence) {
            return callback(new Error('Could not find transaction hash of first message in sequence'));
        }
        ...
        function iterateSeqence(wfMessageSequence) {
            ...
            function iterateMessages(i) {
                ...
                retrieveMultipleMessages(wfQuery, blockchain, function retrieveSequenceRefMessagesCb(err, refMessages) {
                    ...
                    if (wfMessageSequence.length > sequenceBeginLength) {
                        return iterateSeqence(removeSemanticReferences(wfMessageSequence));
                    }
                    // No more referencing messages; clean up sequence and return result
                    return callback(null, removeSupersededMessages(wfMessageSequence));
                });
            }
...
/**
 * Retrieves a single message from datastore or blockchain
 * @private
 * @param {object} wfQuery the query to be performed
 * @param {string} blockchain the blockchain on which the transaction is stored
 * @param {function} callback function called upon completion
 */
function retrieveSingleMessage(wfQuery = {}, blockchain = null, callback) {
    // Lookup message in datastore first - returns an array
    wfApiDatastores.getMessages(wfQuery, function retrieveSingleMessageDbCb(err, wfMessages, count) {
        if (err) return callback(err, wfMessages);

        if (count === 1) return callback(null, wfMessages);
        if (count > 1) return callback(null, wfMessages);

        // If no blockchain specified, do not look further
        if (!blockchain) return callback(null, wfMessages);

        // Lookup message on blockchain - returns a single message, not an array
        wfApiBlockchain.lookupMessage(wfQuery, function retrieveSingleMessageBcCb(err, wfMessage) {
            if (err) return callback(err, null);

            // Found the transaction; try to decode
            wfCodec.decode(wfMessage, function retrieveDecodeCb(err, wfMessage, ivMissing) {
                if (err) return callback(err, null);

                wfMessages = [ wfMessage ];
                if (ivMissing) return callback(new ProcessingError('Cannot decrypt message without initialisation vector', null, ''), wfMessages);

                // If valid message, process further in rx chain to validate and store the message
                if (wfMessage.MetaHeader.formatValid) {
                    wfRxEvent.emit('messageDecoded', wfMessage);
                }
 ...
 * @param {function} callback function called upon completion
 */
function retrieveMultipleMessages(wfQuery = {}, blockchain = null, callback) {
    // Lookup message in database first - returns an array
    wfApiDatastores.getMessages(wfQuery, function retrieveMultiMessagesDbCb(err, wfMessages, count) {
        // Check and return errors or messages found
        if (err) return callback(err, wfMessages);
        if (count > 0) return callback(null, wfMessages);

        // If no blockchain specified, do not look further
        if (!blockchain) return callback(null, wfMessages);

        // Lookup message on blockchain - returns an array
        wfApiBlockchain.queryTransactions(wfQuery, function retrieveMultiMessagesBcCb(err, wfMessages) {
            // Return any error or result from blockchain lookup
            if (err) return callback(err, wfMessages);
            return callback(null, wfMessages);
        });
    });
...
function removeSemanticReferences(wfMessageSequence) {
    ...
    // TODO: check if not Ref Code 2 message are unpurposely removed
