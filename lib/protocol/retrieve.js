...
function getAuthMessages(originatorAddress = '0', blockchain = null, callback) {
    ...
    retrieveMessages(wfQuery, blockchain, function retrieveAuthMessagesCb(err, authSequence) {
        ...
        return callback(null, authSequence);
    });
}
...
function getSequence(transactionHash, blockchain, callback) {
    ...
    if (!blockchain) return callback(new ProcessingError('Need transaction hash to find first message in sequence', null, 'WF_API_BAD_REQUEST'));
    if (!transactionHash) return callback(new ProcessingError('Need blockchain to find first message in sequence', null, 'WF_API_BAD_REQUEST'));
    wfQuery['MetaHeader.transactionHash'] = transactionHash;

    // Call function to retrieve first message of sequence
    retrieveMessages(wfQuery, blockchain, function retrieveSequenceCb(err, initMsgSequence) {
