...
const wfContent = require('./content');
...
function initRx(callback) {
     ...
     * @param {function} callback function passed to next event to call on completion
     */
...
function rxCompleted(wfMessage, callback) {
    log.debug(MODULELOG, `Successfully processed ${type(wfMessage)} message: ` + JSON.stringify(wfMessage));
 ...
 * @private
 * @param {object} err the error that occured in the event chain
 * @param {object} wfMessage a Whiteflag message
 * @param {function} callback function called with error
 */
function rxError(err, wfMessage, callback) {
    ...
    if (err instanceof ProtocolError) {
        if (err.causes) {
            log.debug(MODULELOG, `Protocol error when processing ${messageType}message:` + `${messageStr} ${err.message}: ` + JSON.stringify(err.causes));
        } else {
            log.debug(MODULELOG, `Protocol error when processing ${messageType}message:` + `${messageStr} ${err.message}`);
        }
    } else {
...
function rxGetInitVector(wfMessage, callback) {
    wfState.getQueueData('initVectors', 'referencedMessage', wfMessage.MetaHeader.transactionHash, function rxGetInitVectorQueueCb(err, ivObject) {
        ...
        log.debug(MODULELOG, 'Processed encrypted message without initialisation vector: ' + JSON.stringify(wfMessage));
        wfRxEvent.emit('messageProcessed', wfMessage);
        if (callback) return callback(null, wfMessage);
    });
}
...
/**
 * Calls content validator and emits positive result
 * @private
 * @param {object} wfMessage a Whiteflag message
 * @param {function} callback function passed to next event to call on completion
 * @emits module:lib/protocol/events.rxEvent:contentVerified
 * @emits module:lib/protocol/events.rxEvent:contentSkipped
 */
function rxVerifyContent(wfMessage, callback) {
    // Skip content check for outgoing message if configured
    if (!wfConfigData.rx.verifyContent || wfMessage.MetaHeader.autoGenerated) {
        return wfRxEvent.emit('contentSkipped', wfMessage, callback);
    }
    // Check message content
    wfContent.verify(wfMessage, function rxValidateContntCb(err, wfMessage) {
        // Check for content errors
        if (err && err instanceof ProtocolError) {
            return wfRxEvent.emit('error', err, wfMessage, callback);
        }
        if (err) log.warn(MODULELOG, `Content of incoming ${type(wfMessage)} message could not be verified: ${err.message}`);

        // Emit event upon completion and pass data
        return wfRxEvent.emit('contentVerified', wfMessage, callback);
    });
}
