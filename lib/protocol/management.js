...
function receivedMessage(wfMessage) {
    ...
    switch (wfMessage.MessageHeader.MessageCode) {
        case AUTHMESSAGECODE: {
            switch (wfMessage.MessageBody.VerificationMethod) {
                case '1': {
                    // Authentication Method 1
                    receiveAuthenticationData1(wfMessage);
                    return;
                }
                case '2': {
                    // Authentication Method 2
                    receiveAuthenticationData2(wfMessage);
                    return;
                }
                default: break;
            }
...
function sentMessage(wfMessage) {
    ...
    switch (wfMessage.MessageHeader.MessageCode) {
        case AUTHMESSAGECODE: {
            // Public ECDH key is automatically sent after an authentication message
 ...
 */
function receiveAuthenticationData1(wfAuthMessage) {
    switch (wfAuthMessage.MessageHeader.ReferenceIndicator) {
        ...
        case '2': {
            ...
            wfAuthenticate.verify(wfAuthMessage, function mgmtVerifyAuth1Cb(err, wfMessage) {
                if (err) {
                    if (err instanceof ProtocolError) {
                        log.debug(MODULELOG, `Cannot verify received ${type(wfAuthMessage)} message: ${err.message}`);
                    } else {
                        log.warn(MODULELOG, `Cannot verify received ${type(wfAuthMessage)} message: ${err.message}`);
                    }
                }
                ignore(wfMessage);
            });
        ...
        case '4': {
            ...
            wfAuthenticate.remove(wfAuthMessage, function mgmtRemoveAuth1Cb(err, wfMessage) {
                if (err) {
                    if (err instanceof ProtocolError) {
                        log.debug(MODULELOG, `Cannot verify received ${type(wfAuthMessage)} message: ${err.message}`);
                    } else {
                        log.warn(MODULELOG, `Cannot update originator state after receiving ${type(wfAuthMessage)} message: ${err.message}`);
                    }
                }
                ignore(wfMessage);
            });
...
/**
 * Processes received authentication message for verification method 2
 * @private
 * @param {object} wfAuthMessage Whiteflag authentication message
 * @todo implement authentication method 2
 */
function receiveAuthenticationData2(wfAuthMessage) {
    ignore(wfAuthMessage);
}
...
function receiveInitVector(wfCryptoMessage) {
    ...
    switch (wfCryptoMessage.MessageHeader.ReferenceIndicator) {
        ...
        case '4': {
            ...
            log.trace(MODULELOG, `Received ${type(wfCryptoMessage)} message: Removing initialisation vector from queue`);
            wfState.removeQueueData('initVectors', 'transactionHash', referencedMessage);
            break;
        }
        case '2': {
            // Update iv if on queue
            log.trace(MODULELOG, `Received ${type(wfCryptoMessage)} message: Updating initialisation vector on queue`);
        ...
        case '3': {
            ...
            log.trace(MODULELOG, `Received ${type(wfCryptoMessage)} message with initialisation vector`);
            wfRetrieve.getMessage(referencedMessage, blockchain, function mgmtGetMessageInitVectorCb(err, wfMessages) {
                if (err && !(err instanceof ProcessingError)) log.warn(MODULELOG, `${err.message}`);
...
function sendInitVector(wfMessage) {
    ...
    if (wfMessage.MetaHeader.encryptionInitVector) {
        ...
        // Logging
        log.trace(MODULELOG, `Sending ${type(wfCryptoMessage)} message with initialisation vector for ${type(wfMessage)} message: ${wfMessage.MetaHeader.transactionHash}`);
...
/**
 * Processes received public ECDH key
 * @private
 * @param {object} wfCryptoMessage Whiteflag crypto message with public ECDH key
 */
function receiveECDHpublicKey(wfCryptoMessage) {
    ...
    wfState.getOriginatorData(originatorAddress, function mgmtGetOriginatorCb(err, originator) {
        if (err) return log.error(MODULELOG, `Received ${type(wfCryptoMessage)} message but cannot get originator state to compute shared secret: ${err.message}`);
        ...
        switch (wfCryptoMessage.MessageHeader.ReferenceIndicator) {
            ...
            case '2': {
                ...
                log.trace(MODULELOG, `Received ${type(wfCryptoMessage)} message with public ECDH key from address: ${originatorAddress}`);
                if (originator) {
                    // Known origintaor
                    originator.ecdhPublicKey = originatorECDHpubKey;
                    wfState.updateOriginatorData(originator);
                } else {
                    // Unknown originator
                    const newOriginator = {
                        name: null,
                    ...
                    wfState.updateOriginatorData(newOriginator);
                }
                break;
            }
            case '1':
            case '4': {
                ...
                log.trace(MODULELOG, `Received ${type(wfCryptoMessage)} message to remove public ECDH key from address: ${originatorAddress}`);
                if (originator) {
        ...
        wfState.getBlockchainData(blockchain, function mgmtGetBlockchainDataCb(err, blockchainState) {
            ...
            if (err) return log.error(MODULELOG, `Cannot retrieve ${blockchain} state to compute shared secrets: ${err.message}`);
...
/**
 * Sends an public ECDH key after an authentication message
 * @private
 * @param {object} wfMessage a Whiteflag message
 * @emits _txEvent:messageCommitted
 */
function sendECDHpublicKey(wfAuthMessage) {
    ...
    switch (wfAuthMessage.MessageHeader.ReferenceIndicator) {
        case '0': {
            // An original authentication message resends the public ECDH key if already existing
            newKeyPair = false;
            break;
        }
        case '2': {
    ...
    wfCrypto.getECDHpublicKey(ecdhId, newKeyPair, function mgmtGetMessageInitVectorCb(err, ecdhPublicKey, newKey) {
        if (err) {
            log.error(MODULELOG, `Cannot get and send public ECDH key for account ${address}: ${err.message}`);
            return;
        }
        // Build K message
        const wfCryptoMessage = {
        ...
        // Logging
        log.trace(MODULELOG, `Sending ${type(wfCryptoMessage)} message with public ECDH key`);
        ...
        wfTxEvent.emit('messageCommitted', wfCryptoMessage, function mgmtSendECDHpubKeyCb(err) {
            ...
            if (!newKey) return;
            if (err) return log.warn(MODULELOG, `${type(wfCryptoMessage)} message not sent after renewed ECDH key pair; not computing new secrets`);
            ...
            wfState.getOriginators(function mgmtGetECDHoriginatorsCb(err, originators) {
                if (err) return log.error(MODULELOG, `Cannot get originator state to compute shared secrets: ${err.message}`);
    ...
    wfCrypto.generateECDHsecret(ecdhId, originatorECHDpubKey, function mgmtGenECDHsecretCb(err, secret) {
        if (err) {
            if (err instanceof ProcessingError) {
                return log.warn(MODULELOG, `Cannot compute ECDH negotiated secret for originator address ${originatorAddress}: ${err.message}`);
            }
            return log.error(MODULELOG, `Cannot compute ECDH negotiated secret for originator address ${originatorAddress}: ${err.message}`);
        }
        const secretId = hash(blockchain + address + originatorAddress, KEYIDLENGTH);
        wfState.storeKey('negotiatedKeys', secretId, secret);
        log.debug(MODULELOG, `Computed ECDH negotiated secret for originator address ${originatorAddress}`);
    });
}
