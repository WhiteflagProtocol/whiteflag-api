            ...
            wfAuthenticate.verify(wfAuthMessage, function mgmtVerifyAuthCb(err, wfAuthMessage) {
                if (err) {
                    if (err instanceof ProtocolError) {
                        if (err.causes) {
                            log.debug(MODULELOG, `Cannot verify received ${type(wfAuthMessage)} message ${wfAuthMessage.MetaHeader.transactionHash}: ${err.message}: ` + JSON.stringify(err.causes));
                        } else {
                            log.debug(MODULELOG, `Cannot verify received ${type(wfAuthMessage)} message ${wfAuthMessage.MetaHeader.transactionHash}: ${err.message}`);
                        }
                    } else {
                        log.warn(MODULELOG, `Cannot verify received ${type(wfAuthMessage)} message ${wfAuthMessage.MetaHeader.transactionHash}: ${err.message}`);
                    }
                }
            ...
            wfAuthenticate.remove(wfAuthMessage, function mgmtRemoveAuthCb(err, wfAuthMessage) {
                if (err) {
                    if (err instanceof ProtocolError) {
                        ...
                        log.warn(MODULELOG, `Cannot update originator state after receiving ${type(wfAuthMessage)} message ${wfAuthMessage.MetaHeader.transactionHash}: ${err.message}`);
                    }
                }
                wfRxEvent.emit('messageUpdated', wfAuthMessage);
            });
...
function receiveECDHpublicKey(wfCryptoMessage) {
    ...
    wfState.getOriginatorData(originatorAddress, function mgmtGetOriginatorCb(err, originator) {
        if (err) return log.error(MODULELOG, `Received ${type(wfCryptoMessage)} message ${wfCryptoMessage.MetaHeader.transactionHash} but cannot get originator state to compute shared secret: ${err.message}`);
        ...
        switch (wfCryptoMessage.MessageHeader.ReferenceIndicator) {
            ...
            case '2': {
                ...
                log.trace(MODULELOG, `Received ${type(wfCryptoMessage)} message ${wfCryptoMessage.MetaHeader.transactionHash} with public ECDH key from address ${originatorAddress}`);
                if (originator) {
                    // Known origintaor
            ...
            case '4': {
                ...
                log.trace(MODULELOG, `Received ${type(wfCryptoMessage)} message ${wfCryptoMessage.MetaHeader.transactionHash} to remove public ECDH key from address ${originatorAddress}`);
                if (originator) {
