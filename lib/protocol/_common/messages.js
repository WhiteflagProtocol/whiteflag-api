...
function messageType(wfMessage) {
    ...
    // Check encryption
    let EB = ''; let EE = '';
    if (header.EncryptionIndicator && header.EncryptionIndicator !== '0') {
        EB = '['; EE = ']';
    }
    // Determine message subtype
    if (wfMessage.MessageBody) {
        const { MessageBody: body} = wfMessage;
        if (body.SubjectCode) return `${EB}${header.MessageCode}${body.SubjectCode}${referenceCode}${EE}`;
        if (body.CryptoDataType) return `${EB}${header.MessageCode}${body.CryptoDataType}${referenceCode}${EE}`;
        if (body.VerificationMethod) return `${EB}${header.MessageCode}${body.VerificationMethod}${referenceCode}${EE}`;
        if (body.ResourceMethod) return `${EB}${header.MessageCode}${body.ResourceMethod}${referenceCode}${EE}`;
    }
    return `${EB}${header.MessageCode}${referenceCode}${EE}`;
}
