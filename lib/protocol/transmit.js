 ...
 * @param {Error} err any error
 * @param {wfMessage} wfMessage a Whiteflag message
 * @param {wfMessageCb} callback function on completion
 */
...
function txVerifyMetadata(wfMessage, callback) {
    let msgErrors = checkTxMetaHeaderErrors(wfMessage);
    if (msgErrors.length > 0) {
        let err = new ProtocolError('Invalid message metaheader', msgErrors, 'WF_METAHEADER_ERROR');
...
function txSendMessage(wfMessage, callback) {
    ...
    wfBlockchains.sendMessage(wfMessage, function txBlockchainSendCb(err, wfMessage) {
        let { MetaHeader: meta } = wfMessage;
        if (err) {
            // Retry again for auto generated messages
            if (
                meta.autoGenerated
                && !Object.hasOwn(meta, 'transmissionSuccess')
            ) {
        ...
        if (meta.transmissionSuccess === true) {
...
function checkTxMetaHeaderErrors(wfMessage) {
    ...
    if (!meta.blockchain) msgErrors.push('Blockchain not specified in metaheader');
    if (!meta.originatorAddress) msgErrors.push('Originator address not specified in metaheader');
