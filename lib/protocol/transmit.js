...
function txError(err, wfMessage, callback) {
    ...
    if (err instanceof ProtocolError || err instanceof ProcessingError) {
        if (err.causes) {
            log.debug(MODULELOG, `Cannot process ${messageType}message:` + `${messageStr} ${err.message}: ` + JSON.stringify(err.causes));
        } else {
            log.debug(MODULELOG, `Cannot process ${messageType}message:` + `${messageStr} ${err.message}`);
        }
    } else {
 ...
 * @param {object} wfMessage a Whiteflag message
 * @param {function} callback function passed to next event to call on completion
 * @emits module:lib/protocol/events.txEvent:contentVerified
 * @emits module:lib/protocol/events.txEvent:contentSkipped
 */
function txVerifyContent(wfMessage, callback) {
    // Skip content check for outgoing message if configured
    if (!wfConfigData.tx.verifyContent || wfMessage.MetaHeader.autoGenerated) {
        return wfTxEvent.emit('contentSkipped', wfMessage, callback);
    }
    // Check message content
    wfContent.verify(wfMessage, function txValidateContntCb(err, wfMessage) {
        // Check for content errors
        if (err && err instanceof ProtocolError) {
            return wfTxEvent.emit('error', err, wfMessage, callback);
        }
        if (err) log.warn(MODULELOG, `Content of outgoing ${type(wfMessage)} message could not be verified: ${err.message}`);

        // Emit event upon completion and pass data
        return wfTxEvent.emit('contentVerified', wfMessage, callback);
    });
}
...
function txSendMessage(wfMessage, callback) {
    ...
    if (wfMessage.MessageHeader.MessageCode !== TESTMESSAGECODE
