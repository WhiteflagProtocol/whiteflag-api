    ...
    if (!fs.existsSync(_stateFile)) {
        ...
        wfApiDatastores.getState(function stateReadDatastoreCb(err, stateDbData) {
            ...
            try {
                ...
                return callback(new Error(`Could not extract previous state from datastore: ${err.message}`));
            }
        ...
        fs.readFile(_stateFile, function stateReadFileCb(err, stateFileData) {
            ...
            try {
                ...
                return callback(new Error(`Could not restore previous state from ${_stateFile}: ${err.message}`));
            }
...
function closeState(callback) {
    ...
    wfStateEvent.once('saved', function stateCloseCb() {
        log.info(MODULELOG, 'Saved state before closing');
