 ...
 * @property {string} publicKey The public key associated with the blockchain account of the originator
 ...
 * @property {boolean} authValid Indication whether the originator has been authenticated at some point in time
 * @property {string[]} authMessages Transaction hashes of valid original authentication messages
 */
/**
...
const { hkdf,
        hash,
        zeroise } = require('../_common/crypto');
...
const STATESCHEMADIR = './static/protocol/';
const STATESCHEMAFILE = STATESCHEMADIR + 'state.schema.json';
...
function validateState(stateData = _wfState) {
    let stateErrors = [];
    try {
        stateErrors = arr.pluck(jsonValidate(stateData, _wfStateSchema).errors, 'stack');
    } catch(err) {
