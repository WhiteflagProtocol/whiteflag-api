...
function getBlockchainData(blockchain, callback) {
    if (!_wfState.blockchains.hasOwnProperty(blockchain)) {
...
function updateOriginatorData(data) {
    if (!data.hasOwnProperty('address')) {
        return log.error(MODULELOG, 'Cannot add or update originator data without address');
    }
...
function getQueue(queue, callback) {
    if (!_wfState.queue.hasOwnProperty(queue)) {
...
function getKey(category, id, callback) {
    if (!_wfState.crypto.hasOwnProperty(category)) {
...
function extractState(stateObject) {
    ...
    if (stateObject.hasOwnProperty('tag') && stateObject.hasOwnProperty('iv')) {
...
function checkState(stateData) {
    ...
    if (!stateData.hasOwnProperty('blockchains')) stateData.blockchains = {};
    if (!stateData.hasOwnProperty('originators')) stateData.originators = [];
    if (!stateData.hasOwnProperty('queue')) stateData.queue = {};
    if (!stateData.queue.hasOwnProperty('initVectors')) stateData.queue.initVectors = [];
    if (!stateData.queue.hasOwnProperty('blockDepths')) stateData.queue.blockDepths = [];
    if (!stateData.hasOwnProperty('crypto')) stateData.crypto = {};
    if (!stateData.crypto.hasOwnProperty('blockchainKeys')) stateData.crypto.blockchainKeys = [];
    if (!stateData.crypto.hasOwnProperty('ecdhPrivateKeys')) stateData.crypto.ecdhPrivateKeys = [];
    if (!stateData.crypto.hasOwnProperty('negotiatedKeys')) stateData.crypto.negotiatedKeys = [];
    if (!stateData.crypto.hasOwnProperty('presharedKeys')) stateData.crypto.presharedKeys = [];
    if (!stateData.crypto.hasOwnProperty('authTokens')) stateData.crypto.authTokens = [];

    // Put old private keys in keystore
    const blockchains = Object.keys(stateData.blockchains);
    for (let b = 0; b < blockchains.length; b++) {
        if (stateData.blockchains[blockchains[b]].hasOwnProperty('accounts')) {
            const accounts = stateData.blockchains[blockchains[b]].accounts;
            for (let a = 0; a < accounts.length; a++) {
                if (accounts[a].hasOwnProperty('privateKey')) {
