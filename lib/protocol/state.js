...
const wfStateSchema = JSON.parse(fs.readFileSync('./lib/protocol/static/state.schema.json'));
...
let _stateFile = 'whiteflag.state';
 ...
 * @param {function} callback function called after initialising/restoring state
...
function initState(callback) {
    ...
    if (wfConfigData.state.saveToFile === true) _saveToFile = true;
    _stateFile = wfConfigData.state.saveToFile || _stateFile;

    // Remove master key from memory
    delete wfConfigData.state.masterKey;

    // Check if state file exists
    if (!fs.existsSync(_stateFile)) {
        // Get the state from the datastore
        log.trace(MODULELOG, 'Getting state from datastore');
        wfApiDatastores.getState(function stateReadDatastoreCb(err, stateDbData) {
            if (err) return callback(new Error(`Could not retrieve previous state from datastore: ${err.message}`));
            if (!stateDbData) {
                log.info(MODULELOG, 'State does not exist in datastore; starting with clean internal state');
            ...
            try {
                ...
                return callback(new Error(`Could not extract previous state from datastore data: ${err.message}`));
            }
        ...
        log.trace(MODULELOG, `State file ${_stateFile} exists; getting state from file`);

        // Make a copy of the state file
        let stateFileBak = _stateFile + '.' + 'bak';
        fs.copyFile(_stateFile, stateFileBak, err => {
            if (err) log.error(MODULELOG, `Cannot backup state file ${_stateFile}: ${err.message}`);
                else log.info(MODULELOG, `State file ${_stateFile} backed up to ${stateFileBak}`);
        });
        // Read saved state from file
        fs.readFile(_stateFile, function stateReadFileCb(err, stateFileData) {
            if (err) {
                return callback(new Error(`Could not restore state from ${_stateFile}: ${err.message}`));
            }
            try {
                // Extract and decrypt state
                _wfState = extractState(JSON.parse(stateFileData));
            } catch(err) {
                return callback(new Error(`Could not restore previous state: ${err.message}`));
            }
            log.info(MODULELOG, `Restored previous state from ${_stateFile}`);

            // Remove file if it is not used anymore
            if (!_saveToFile) {
                log.trace(MODULELOG, `Removing ${_stateFile}`);
                fs.unlink(_stateFile, function stateRemoveFileCb(err) {
                    if (err) log(MODULELOG, `Could not remove ${_stateFile}: ${err.message}`);
                });
            }
            // Done restoring state
            log.info(MODULELOG, `Restored previous state from ${_stateFile}`);
...
function closeState(callback) {
    wfStateEvent.once('saved', function stateCloseCb() {
        ...
        log.info(MODULELOG, 'Closed state');
...
function saveState() {
    ...
    wfApiDatastores.storeState(stateObject, function stateStoreDatastoreCb(err) {
        if (err) {
            return log.error(MODULELOG, `Could not store state in datastore: ${err.message}`);
        }
    ...
    if (_saveToFile) {
        ...
        fs.writeFile(_stateFile, JSON.stringify(stateObject, null, 2), function stateWriteFileCb(err) {
            if (err) {
                return log.error(MODULELOG, `Could not save current state to ${_stateFile}: ${err.message}`);
            }
            return wfStateEvent.emit('saved');
        });
    }
 ...
 * @param {function} callback function to which data is passed
 */
...
function backupAccount(blockchain, address) {
    ...
    if (account) {
        ...
        fs.writeFile(accountFile, JSON.stringify(account, null, 2), function stateWriteAccountFileCb(err) {
            if (err) {
                return log.error(MODULELOG, `Could not save blockchain account to ${accountFile}: ${err.message}`);
            }
...
function getOriginatorData(address, callback) {
    const index = _wfState.originators.findIndex(originator => originator.address.toLowerCase() === address.toLowerCase());
    if (index < 0) return callback(null, null);
    return callback(null, _wfState.originators[index]);
}

/**
 * Insert or update an originator
 * @function updateOriginatorData
 * @alias module:lib/protocol/state.updateOriginatorData
 ...
 */
function updateOriginatorData(data) {
    if (Object.prototype.hasOwnProperty.call(data, 'address')) {
        // Check if originator with the same address already exists and either insert or update
        const index = _wfState.originators.findIndex(originator => originator.address.toLowerCase() === data.address.toLowerCase());
        if (index < 0) {
            _wfState.originators.push(data);
            wfStateEvent.emit('insertedOriginator', data.address);
        } else {
            // Replace with new data
            _wfState.originators[index] = data;
            wfStateEvent.emit('updatedOriginator', data.address);
        }
    } else {
        // Allow originator without address only with authentication token
        ...
        data.address = '';
        const index = _wfState.originators.findIndex(originator => originator.authTokenId.toLowerCase() === data.authTokenId.toLowerCase());
        if (index < 0) {
            _wfState.originators.push(data);
            wfStateEvent.emit('insertedOriginatorAuthData', data.address);
        } else {
            // Replace with new data
            object.update(data, _wfState.originators[index]);
            wfStateEvent.emit('updatedOriginatorAuthData', data.address);
        }
    }
    return saveState();
}
 ...
 * @function removeOriginatorAuthData
 * @alias module:lib/protocol/state.removeOriginatorData
 * @param {string} address the originator's blockchain address
 * @emits module:lib/protocol/events.stateEvent:removedOriginator
 */
function removeOriginatorAuthData(authTokenId) {
    const index = _wfState.originators.findIndex(originator => originator.authTokenId.toLowerCase() === authTokenId.toLowerCase());
    if (index >= 0) {
        if (!Object.prototype.hasOwnProperty.call(_wfState.originators[index], 'address') || _wfState.originators[index].address === '') {
            _wfState.originators.splice(index, 1);
            wfStateEvent.emit('removedOriginatorAuthData', authTokenId);
        } else {
            _wfState.originators[index].authTokenId = undefined;
            wfStateEvent.emit('removedOriginatorAuthToken', _wfState.originators[index].address);
        }
        return saveState();
    }
}
...
function getQueueData(queue, property, value, callback) {
    ...
    const index = _wfState.queue[queue].findIndex(item => item[property].toLowerCase() === value.toLowerCase());
    if (index < 0) return callback(null, null);
    return callback(null, _wfState.queue[queue][index]);
}

/**
 * Puts or updates specific item in specific queue
 * @function updateQueueData
 * @alias module:lib/protocol/state.updateQueueData
 * @param {string} queue the name of the queue
 * @param {string} property the data object property name by which the item is identified
 ...
 */
function updateQueueData(queue, property, data) {
    const index = _wfState.queue[queue].findIndex(item => item[property].toLowerCase() === data[property].toLowerCase());
    if (index < 0) {
 ...
 * @param {function} callback function to which the key is passed
 */
function getKey(category, id, callback) {
    ...
    const index = _wfState.crypto[category].findIndex(item => item.id.toLowerCase() === id.toLowerCase());
    if (index < 0) return callback(null, null);
...
/**
 * Stores or updates a cryptographic key
 * @function updateKey
 * @alias module:lib/protocol/state.updateKey
 * @param {string} category the key category
 * @param {string} id unique identifier of the key
 * @param {string} key the key to be stored or updated in raw hexadecimal format
 */
function storeKey(category, id, key) {
    const index = _wfState.crypto[category].findIndex(item => item.id.toLowerCase() === id.toLowerCase());
...
function encloseState(stateData = _wfState) {
    let stateObject = {};
    if (_encryption) {
        if (!_masterEncryptionKey) return log.debug(MODULELOG, 'No master encryption key');
        const keyBuffer = Buffer.from(_masterEncryptionKey, BINENCODING);
        stateObject = encryptState(JSON.stringify(stateData), keyBuffer);
    } else {
...
function extractState(stateObject) {
    ...
    if (!Object.prototype.hasOwnProperty.call(stateObject, 'state')) {
        log.fatal(MODULELOG, 'State object does not contain state property and might be corrupted');
        return process.exit(1);
    }
    if (Object.prototype.hasOwnProperty.call(stateObject, 'tag') && Object.prototype.hasOwnProperty.call(stateObject, 'iv')) {
        stateDataStr = decryptState(stateObject);
    } else {
        stateDataStr = stateObject.state;
    }
    let stateData = checkState(JSON.parse(stateDataStr));
...
function checkState(stateData) {
    ...
    // Put old private keys in keystore
    const blockchains = Object.keys(stateData.blockchains);
    for (let b = 0; b < blockchains.length; b++) {
        if (Object.prototype.hasOwnProperty.call(stateData.blockchains[blockchains[b]], 'accounts')) {
            const accounts = stateData.blockchains[blockchains[b]].accounts;
            for (let a = 0; a < accounts.length; a++) {
                if (Object.prototype.hasOwnProperty.call(accounts[a], 'privateKey')) {
                    const id = hash(blockchains[b] + accounts[a].address, KEYIDLENGTH);
                    const index = stateData.crypto.blockchainKeys.findIndex(item => item.id.toLowerCase() === id.toLowerCase());
                    const data = encryptKey(accounts[a].privateKey, id);
                    if (index < 0) {
