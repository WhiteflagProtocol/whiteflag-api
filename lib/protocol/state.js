...
function upsertOriginatorData(data) {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'address')) {
        indexA = _wfState.originators.findIndex(originator => originator.address.toLowerCase() === data.address.toLowerCase());
    }
    let indexT = null;
    if (Object.prototype.hasOwnProperty.call(data, 'authTokenId')) {
        indexT = _wfState.originators.findIndex(originator => originator.authTokenId === data.authTokenId);
    }
    ...
    if (indexA < 0 || indexA === null) {
        // No address; check if not also existing originator with token    
        if (indexT < 0 || indexT === null) {
            // Insert if no originator with token or address
            if (indexT === null) data.authTokenId = 'NA';
            if (indexA === null) data.address = 'unknown';
            _wfState.originators.push(data);
            if (indexT === null) wfStateEvent.emit('insertedOriginator', data.address);
            if (indexA === null) wfStateEvent.emit('insertedAuthenticationToken', data.authTokenId);
            return saveState();
        } else {
            // Update if existing originator with token but no address
            object.update(data, _wfState.originators[indexT]);
            wfStateEvent.emit('updatedAuthenticationToken', data.authTokenId);
            return saveState();
        }
    } else {
        // Originator with address; also check if not also existing originator with token    
        if (indexT < 0 || indexT === null) {
            // Update if existing originator with address but no token
            object.update(data, _wfState.originators[indexA]);
            wfStateEvent.emit('updatedOriginator', data.address);
            return saveState();
        }
    }
    // Check if existing address and token are the same originator
    if (indexA === indexT) {
        // Update existing originator with the same address and token
        object.update(data, _wfState.originators[indexA]);
        wfStateEvent.emit('updatedOriginator', data.address);
        return saveState();
    } else {
        // Merge existing originator token into originatro entry with address
        object.update(_wfState.originators[indexT], _wfState.originators[indexA]);
 ...
 * @function getOriginatorByToken
 * @alias module:lib/protocol/state.getOriginatorByToken
 * @param {string} authTokenId the originator's blockchain address
 * @param {function} callback function to which data is passed
 */
function getOriginatorAuthToken(authTokenId, callback) {
    const index = _wfState.originators.findIndex(originator => originator.authTokenId === authTokenId);
    if (index < 0) return callback(null, null);
    return callback(null, _wfState.originators[index]);
}
...
function removeOriginatorAuthToken(authTokenId) {
    ...
    if (index >= 0) {
        if (!Object.prototype.hasOwnProperty.call(_wfState.originators[index], 'address') || _wfState.originators[index].address === '') {
            _wfState.originators.splice(index, 1);
            wfStateEvent.emit('removedAuthenticationToken', authTokenId);
        } else {
        ...
        if (Object.prototype.hasOwnProperty.call(stateData.blockchains[blockchains[b]], 'accounts')) {
            ...
            for (let a = 0; a < accounts.length; a++) {
                if (Object.prototype.hasOwnProperty.call(accounts[a], 'privateKey')) {
                    ...
                    accounts[a].privateKey = undefined;
                }
            }
        }
    }
