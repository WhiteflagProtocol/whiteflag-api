...
function initState(callback) {
    ...
    _stateFile = wfConfigData.state.file || _stateFile;
    ...
    if (!fs.existsSync(_stateFile)) {
        ...
        wfApiDatastores.getState(function stateReadDatastoreCb(err, stateDbData) {
            ...
            // Notify if configured to use state file
            if (_saveToFile) {
                log.info(MODULELOG, `Configured to use file to save state: ${_stateFile}`);
            }
        ...
        log.trace(MODULELOG, `Getting previous state from existing state file: ${_stateFile}`);

        // Read saved state from file
        fs.readFile(_stateFile, function stateReadFileCb(err, stateFileData) {
            if (err) return callback(new Error(`Could not read state from ${_stateFile}: ${err.message}`));

            // Extract and decrypt state
            try {
                _wfState = extractState(JSON.parse(stateFileData));
            } catch(err) {
                return callback(new Error(`Could not extract state from ${_stateFile}: ${err.message}`));
            }
            ...
            fs.copyFile(_stateFile, stateFileBak, err => {
                if (err) log.error(MODULELOG, `Could not backup state file ${_stateFile}: ${err.message}`);
                    else log.info(MODULELOG, `State file backed up to ${stateFileBak}`);

                // Remove file if it is not used anymore
                if (!_saveToFile) {
                    log.info(MODULELOG, 'Configured to use datastore to save state');
                    fs.unlink(_stateFile, function stateRemoveFileCb(err) {
                        if (err) log.error(MODULELOG, `Could not remove state file ${_stateFile}: ${err.message}`);
                            else log.info(MODULELOG, `State file ${_stateFile} not in use and deleted`);
                    });
                }
                // Done restoring state
                saveState();
...
function checkState(stateData) {
    ...
    // Check and correct old data structures in blockchain state
    ...
    for (let b = 0; b < blockchains.length; b++) {
        if (Object.prototype.hasOwnProperty.call(stateData.blockchains[blockchains[b]], 'accounts')) {
            ...
            for (let a = 0; a < accounts.length; a++) {
                // Some versions have an invalid account entry with a non-string address
                if (Object.prototype.hasOwnProperty.call(accounts[a], 'address')) {
                    if (typeof accounts[a].address !== 'string') {
                        accounts.splice(a, 1);
                        continue;
                    }
                }
                // Put naked private keys in keystore
