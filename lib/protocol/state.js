...
function upsertOriginatorData(data) {
    if (Object.prototype.hasOwnProperty.call(data, 'address')) {
        ...
        if (index < 0) {
            _wfState.originators.push(data);
            wfStateEvent.emit('insertedAuthenticationToken', data.address);
        } else {
            // Replace with new data
            object.update(data, _wfState.originators[index]);
            wfStateEvent.emit('updatedAuthenticationToken', data.address);
        }
    }
    return saveState();
}
...
/**
 * Inserts or updates originator authentication token
 * @function upsertAuthenticationToken
 */
// TODO: Add function
 ...
 * @alias module:lib/protocol/state.removeAuthenticationToken
 * @param {string} address the originator's blockchain address
 * @emits module:lib/protocol/events.stateEvent:removedOriginator
 */
function removeAuthenticationToken(authTokenId) {
