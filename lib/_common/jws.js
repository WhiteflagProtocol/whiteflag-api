...
/**
 * Creates the compact serialized string from input and signature
 * @param {string} input the compact serialized header and payload
 * @param {string} signature the signature
 * @returns {string} a JWS as a compact serialized string
 */
function createCompact(input, signature) {
    return (input + JWSSEPARATOR + signature);
}
...
function createFlattened(input, signature) {
    return toFlattened(createCompact(input, signature));
}
...
function serializeSignInput(jws) {
    ...
    return (jwsFlat.protected + JWSSEPARATOR + jwsFlat.payload);
}
