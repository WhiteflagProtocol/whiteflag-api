...
function decode(jws) {
    let jwsErrors = [];
    if (!isString(jws?.protected)) jwsErrors.push('Missing or incorrect property: \'protected\'');
    if (!isString(jws?.payload)) jwsErrors.push('Missing or incorrect property: \'payload\'');
    if (!isString(jws?.signature)) jwsErrors.push('Missing or incorrect property: \'signature\'');
...
function toCompact(jws) {
    ...
    switch (type(jws)) {
        ...
        default: {
            throw new ProcessingError('Invalid JWS', null, 'WF_API_BAD_REQUEST');
        }
    }
    return jwsCompact;
}
