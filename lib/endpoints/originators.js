...
function storeAuthToken(req, res, operationId, callback) {
    ...
    const keyId = hash(originatorData.name + originatorData.blockchain + token, KEYIDLENGTH);
    wfState.getKey('authTokens', keyId, function storeAuthTokenGetKeyCb(err, existingKey) {
        if (!err && existingKey) {
            existingKey = undefined;
            err = new ProcessingError('Authentication token already exists', null, 'WF_APT_RESOURCE_CONFLICT');
        }
        if (!err) {
            originatorData.authTokenId = keyId;
            originatorData.authenticationValid = false;
            wfState.upsertOriginatorData(originatorData);
            wfState.upsertKey('authTokens', keyId, token);
            token = undefined;
        }
        // Add metadata and send response
        resBody.meta.resource = `/originators/tokens/${keyId}`;
        response.sendImperative(res, err, resBody, originatorData, callback);
    });
}
