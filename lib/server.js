 ...
 * @param {function} callback function called after stopping the server
 */
    ...
    try {
        ...
        endpoints.forEach(function endpointsCb(endpoint) {
            if (!Object.prototype.hasOwnProperty.call(_wfApiConfig.endpoints, endpoint[OPERATIONID])
                ...
                log.trace(MODULELOG, `Disabled endpoint: ${endpoint[OPERATIONID]}`);
            }
        });
...
function sendSocket(data) {
    if (!_wfSocket) {
        return log.trace(MODULELOG, 'Cannot send data: Socket not initialised');
    }
