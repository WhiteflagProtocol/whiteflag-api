...
function createEndpoints(endpointCb, callback) {
    ...
    try {
        ...
        initOriginatorsEndpoints(endpointCb);
        initSignaturesEndpoints(endpointCb);
        initTokensEndpoints(endpointCb);
 ...
 * @param {logEndpointEventCb} endpointCb function passed to the endpoint handler to be invoked when done
 */
function initOriginatorsEndpoints(endpointCb) {
    ...
    createEndpoint('/originators/tokens/:authTokenId', 'GET', 'getAuthToken', wfApiOriginatorsHandler.getAuthToken, endpointCb);
    if (_wfApiConfig.endpoints.storeAuthToken) {
        createEndpoint('/originators/tokens', 'POST', 'storeAuthToken', wfApiOriginatorsHandler.storeAuthToken, endpointCb);
    }
    if (_wfApiConfig.endpoints.deleteAuthToken) {
        createEndpoint('/originators/tokens/:authTokenId', 'DELETE', 'deleteAuthToken', wfApiOriginatorsHandler.deleteAuthToken, endpointCb);
    }
}
...
/**
 * Enables tokens endpoints
 * @private
 * @param {logEndpointEventCb} endpointCb function passed to the endpoint handler to be invoked when done
 */
function initTokensEndpoints(endpointCb) {
    // Tokens endpoints
    createEndpoint('/token/create', 'POST', 'createToken', wfApiTokensHandler.create, endpointCb);
}
