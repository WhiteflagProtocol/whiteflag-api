...
const { createBody,
        sendImperative } = require('./operations/_common/response');

/* Whiteflag modules */
const wfConfig = require('./config');

/* Whiteflag modules with endpoint operations */
const wfApiSingletonsHandler = require('./operations/singletons');
...
const wfApiQueuesHandler = require('./operations/queues');
...
const PORT_LOWEST = 1
const PORT_HIGHEST = 65536

/**
 * @constant {Array} wfApiEndpoints
 * @todo Restructure endpoints (#)
 */
const wfApiEndpoints = [
 ...
 * @param {genericCb} callback function called after starting the server
 */
function startServer(callback) {
    // Get configuration
    _wfApiConfig = wfConfig.getConfig();
    ...
    if (port < PORT_LOWEST || port > PORT_HIGHEST) {
 ...
 * @param {errorCb} callback function called after stopping the server
 */
...
function createEndpoints(endpointCb, callback) {
    ...
    try {
        ...
        for (const endpoint of wfApiEndpoints) {
            if (!Object.hasOwn(_wfApiConfig.endpoints, endpoint[OPERATIONID])
 ...
 * @param {string} operationId the operation id as defined in the openapi definition
 * @param {logEndpointEventCb} callback function called on completion
 */
...
function errorHandler(err, req, res, next) {
    ...
    let resBody = createBody(req, null);
