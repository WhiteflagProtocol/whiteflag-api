'use strict';
/**
 * @module test/common/messages
 * @summary Common message functions test script
...
testCase('Common messages module', function() {
    testCase('Determining message types', function() {
        assertion(' 1. should return "NULL" if no message', function(done) {
            let messageType = msg.type(testVector['1'].message);
        ...
        assertion(' 8. should return correct authentication message type without subtype', function(done) {
            let messageType = msg.type(testVector['8'].message);
            assert.strictEqual(messageType, testVector['8'].type);
            return done();
        });
        assertion(' 9. should return correct resource message type', function(done) {
            let messageType = msg.type(testVector['9'].message);
            assert.strictEqual(messageType, testVector['9'].type);
            return done();
        });
    });
    testCase('Determining message titles', function() {
        assertion(' 1. should correctly identify status message', function(done) {
            let messageTitle = msg.title(testVector['5'].message);
            assert.strictEqual(messageTitle, "StatusSignal");
            return done();
        });
        assertion(' 2. should correctly identify crypto message', function(done) {
            let messageTitle = msg.title(testVector['7'].message);
            assert.strictEqual(messageTitle, "Crypto");
            return done();
        });
        assertion(' 3. should correctly identify authentication message', function(done) {
            let messageTitle = msg.title(testVector['8'].message);
            assert.strictEqual(messageTitle, "Authentication");
            return done();
        });
    });
});
